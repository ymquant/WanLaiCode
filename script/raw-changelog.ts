...
// fork 默认仓库；忘设 GH_REPO 时不要打上游 API（上游不存在 fork 的 tag，会触发漫长重试）
const repo = process.env.GH_REPO ?? "ymquant/wanlaicodex"
...
`)
  ...
  // 私有仓库：清 author 跳过 (@xxx) 署名，传空 thanks 跳过 contributors 区块
  for (const c of list) c.author = null
