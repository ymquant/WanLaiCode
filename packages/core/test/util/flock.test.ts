...
describe("util.flock", () => {
  ...
  test("does not reclaim a live owner whose heartbeat went stale", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, "locks")
    const key = "flock:live-starved"
    const ready = path.join(tmp.path, "ready")

    // Owner holds the lock but freezes its event loop past staleMs, so the
    // heartbeat stops refreshing even though the process is alive.
    const proc = spawnWorker({
      key,
      dir,
      ready,
    ...
    try {
      ...
      // A contender must NOT break the lock: the owner is starved, not dead.
      const res = await Flock.acquire(key, { dir, staleMs: 500, timeoutMs: 1_500 })
        .then((lease) => ({ acquired: true as const, lease }))
        .catch((err) => ({ acquired: false as const, err }))

      if (res.acquired) await res.lease.release()
      expect(res.acquired).toBe(false)
      if (res.acquired) throw new Error("contender wrongly reclaimed a live owner")
      expect(res.err).toBeInstanceOf(Error)
      expect((res.err as Error).message).toContain("Timed out")
    } finally {
      await stopWorker(proc).catch(() => undefined)
      await new Promise((resolve) => proc.on("close", resolve))
    }
  ...
  test("refuses token mismatch release and recovers from stale", async () => {
    ...
    const err = await Flock.withLock(
      ...
      async () => {
        ...
        json.token = "tampered"
        // Model a real takeover: another owner rewrote the metadata. Marking it
        // as a host we cannot verify means recovery falls back to heartbeat
        // staleness — without this the live local-PID guard would (correctly)
        // refuse to reclaim a lock whose recorded owner is still alive.
        ;(json as { hostname?: string }).hostname = "flock-test-other-host"
