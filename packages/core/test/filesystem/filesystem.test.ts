...
import { Effect, Exit, Layer, FileSystem } from "effect"
...
describe("AppFileSystem", () => {
  ...
  describe("readJson / writeJson", () => {
    ...
    it(
      "损坏 JSON 抛 typed FileSystemError(method=readJson) 而非 defect",
      Effect.gen(function* () {
        const fs = yield* AppFileSystem.Service
        const filesys = yield* FileSystem.FileSystem
        const tmp = yield* filesys.makeTempDirectoryScoped()
        const file = path.join(tmp, "broken.json")
        yield* filesys.writeFileString(file, "{ not json")

        const exit = yield* Effect.exit(fs.readJson(file))
        expect(Exit.isFailure(exit)).toBe(true)
        const err = yield* fs.readJson(file).pipe(Effect.flip)
        expect((err as any)._tag).toBe("FileSystemError")
        expect((err as any).method).toBe("readJson")
      }),
    )

    it(
      "writeJson 原子写：round-trip 正确且不残留临时文件",
      Effect.gen(function* () {
        const fs = yield* AppFileSystem.Service
        const filesys = yield* FileSystem.FileSystem
        const tmp = yield* filesys.makeTempDirectoryScoped()
        const file = path.join(tmp, "cred.json")
        const data = { token: "secret", n: 1 }

        yield* fs.writeJson(file, data, 0o600)
  ...
  describe("writeWithDirs", () => {
    ...
    it(
      ...
      Effect.gen(function* () {
        ...
        yield* fs.writeWithDirs(file, "OLD")
        yield* fs.writeWithDirs(file, "NEW-CONTENT")

        expect(yield* filesys.readFileString(file)).toBe("NEW-CONTENT")
        const entries = yield* filesys.readDirectory(tmp)
        expect(entries.some((e) => e.endsWith(".tmp"))).toBe(false)
      }),
    )
