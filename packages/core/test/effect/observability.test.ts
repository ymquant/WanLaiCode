...
const opencodeClient = (process.env.WANLAICODE_CLIENT ?? process.env.OPENCODE_CLIENT)

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) {
    delete process.env.WANLAICODE_CLIENT
    delete process.env.OPENCODE_CLIENT
  } else process.env.WANLAICODE_CLIENT = opencodeClient
})
...
describe("resource", () => {
  ...
  test("keeps built-in attributes when env values conflict", () => {
    process.env.WANLAICODE_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
