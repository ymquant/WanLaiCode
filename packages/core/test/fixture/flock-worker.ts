...
// Synchronously freeze the event loop so the heartbeat timer cannot fire,
// simulating a scheduler-starved or suspended (but still alive) owner.
function starve(ms: number) {
...
async function job(input: Msg) {
  ...
  try {
    if (input.starveMs && input.starveMs > 0) {
      starve(input.starveMs)
    }
