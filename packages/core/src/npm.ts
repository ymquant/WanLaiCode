...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const install: Interface["install"] = Effect.fn("Npm.install")(function* (dir, input) {
      // 测试钩子：OPENCODE_DISABLE_NPM_INSTALL=true 时跳过实际 reify。Windows 上
      // arborist.reify 偶发 hang 90s+，会让 Plugin.init / Config.waitForDependencies 卡死。
      if (Flag.WANLAICODE_DISABLE_NPM_INSTALL) return
