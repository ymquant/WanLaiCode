  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const writeWithDirs = Effect.fn("FileSystem.writeWithDirs")(function* (
        ...
        const writeTmp =
          typeof content === "string" ? fs.writeFileString(tmp, content) : fs.writeFile(tmp, content)
        ...
        // 原子 rename 会按目录权限覆盖目标，绕过目标文件自身的只读权限；
        // 这里在提交前显式校验：目标已存在且不可写时清理临时文件并失败。
        if (yield* fs.exists(path)) {
          yield* fs.access(path, { writable: true }).pipe(Effect.tapError(() => fs.remove(tmp).pipe(Effect.ignore)))
        }
        yield* fs.rename(tmp, path).pipe(Effect.tapError(() => fs.remove(tmp).pipe(Effect.ignore)))
      })
