...
export namespace Flock {
  ...
  async function owner(metaPath: string) {
    try {
  ...
  function alive(meta: { pid?: number; hostname?: string } | undefined) {
    // Only the local host's PIDs are meaningful here; a different host's PID
    // tells us nothing, so defer to heartbeat-based staleness in that case.
    if (!meta || meta.hostname !== os.hostname() || typeof meta.pid !== "number") return false
    try {
  ...
  async function stale(lockDir: string, heartbeatPath: string, metaPath: string, staleMs: number) {
    ...
    if (heartbeat) {
      expired = now - heartbeat.mtimeMs > staleMs
    } else {
      const meta = await stats(metaPath)
      if (meta) {
        expired = now - meta.mtimeMs > staleMs
      } else {
        const dir = await stats(lockDir)
        if (!dir) return false
        expired = now - dir.mtimeMs > staleMs
      }
    }

    if (!expired) return false

    // A lapsed heartbeat proves only that the owner stopped refreshing it, not
    // that it crashed: under heavy load or process suspension a live owner can
    // miss heartbeats for a long time. Reclaiming such a lock lets a second
    // contender enter the critical section concurrently, so never break a lock
    // whose owner process is still alive on this host.
    if (alive(await owner(metaPath))) return false
