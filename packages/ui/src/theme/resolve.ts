...
export function resolveThemeVariant(variant: ThemeVariant, isDark: boolean): ResolvedTheme {
  ...
  if (!("markdown-code-background" in overrides)) {
    const bg = getHex(tokens["background-base"])
    const fg = getHex(tokens["text-strong"])
    tokens["markdown-code-background"] =
      bg && fg
        ? blend(fg, bg, isDark ? 0.16 : 0.07)
        : (`color-mix(in srgb, var(--background-base) ${isDark ? 84 : 93}%, var(--text-strong) ${isDark ? 16 : 7}%)` as ColorValue)
  }
