...
export function Popover<T extends ValidComponent = "div">(props: PopoverProps<T>) {
  ...
  createEffect(() => {
    ...
    // 延后一帧注册外部点击/焦点监听:让本次「开启」交互的残留事件(portal 内容挂载、
    // Kobalte 自动聚焦、模态 dialog focus-trap 把焦点拽回)先走完,否则这些事件会在
    // contentRef 尚未就绪时被误判为「外部」而立即关闭弹层 —— 即"弹出又关掉再点没反应"。
    const removers: Array<() => void> = []
    const raf = requestAnimationFrame(() => {
      removers.push(makeEventListener(window, "keydown", onKeyDown, { capture: true }))
      removers.push(makeEventListener(window, "pointerdown", onPointerDown, { capture: true }))
      removers.push(makeEventListener(window, "focusin", onFocusIn, { capture: true }))
    })
    onCleanup(() => {
