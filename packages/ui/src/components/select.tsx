...
export type SelectProps<T> = Omit<ComponentProps<typeof Kobalte<T>>, "value" | "onSelect" | "children"> & {
  ...
  current?: T
  value?: (x: T) => string
  label?: (x: T) => string | JSX.Element
  /** Plain string for filtering / `optionTextValue` when `label` returns JSX. */
  optionTextValue?: (x: T) => string
  optionDisabled?: (x: T) => boolean
  groupBy?: (x: T) => string
...
export function Select<T>(props: SelectProps<T> & Omit<ButtonProps, "children">) {
  ...
  const textValueFor = (item: T) =>
    local.optionTextValue?.(item) ??
    (local.label
      ? (() => {
          const rendered = local.label(item)
          return typeof rendered === "string" ? rendered : keyFor(item)
        })()
      : keyFor(item))
  ...
  return (
    ...
    <Kobalte<T, { category: string; options: T[] }>
      ...
      optionTextValue={textValueFor}
      optionDisabled={local.optionDisabled}
      ...
      itemComponent={(itemProps) => (
        <Kobalte.Item
          ...
          onPointerEnter={() => {
            if (local.optionDisabled?.(itemProps.item.rawValue)) return
      ...
      onChange={(v) => {
        if (v && local.optionDisabled?.(v)) return
    ...
    >
      <Kobalte.Trigger
        ...
        style={{ "-webkit-app-region": "no-drag", ...local.triggerStyle } as JSX.CSSProperties}
        classList={{
          ...local.classList,
          [local.class ?? ""]: !!local.class,
        }}
