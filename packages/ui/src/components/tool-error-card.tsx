import { type ComponentProps, createMemo, type JSX, Show, splitProps } from "solid-js"
...
export function ToolErrorCard(props: ToolErrorCardProps) {
  ...
  const [split, rest] = splitProps(props, ["tool", "error", "defaultOpen", "subtitle", "href", "children"])
  const name = createMemo(() => {
    const map: Record<string, string> = {
      ...
      // 生图拒绝态使用面向用户的名称，避免在升级卡中暴露内部 snake_case 工具标识。
      image_generation: "ui.tool.imageGeneration",
    }
      ...
      >
        ...
        <Collapsible.Content>
          <div data-slot="tool-error-card-content">
            ...
            {/* 套餐拒绝等可恢复错误可在标准错误详情下追加结构化操作，同时复用同一折叠与复制行为。 */}
            {split.children}
          </div>
        </Collapsible.Content>
      </Collapsible>
