...
import { createEffect, createMemo, For, type JSX, on, Show, untrack } from "solid-js"
...
export function List<T>(props: ListProps<T> & { ref?: (ref: ListRef) => void }) {
  ...
  const invalidateClip = () => setStore("clipVersion", (value) => value + 1)
  const scheduleClip = () => requestAnimationFrame(invalidateClip)
  const setScrollRef = (el: HTMLDivElement | undefined) => {
    setStore("scrollRef", el)
    if (!el) return
    scheduleClip()
  }
  createResizeObserver(scrollRef, scheduleClip)
  const clipped = createMemo(() => {
    store.scrollTop
    store.clipVersion
    const scroll = scrollRef()
    if (!props.hidePartialItems || !scroll) return { items: new Set<string>(), headers: new Set<string>() }
    const rect = scroll.getBoundingClientRect()
    const headers = Array.from(scroll.querySelectorAll<HTMLElement>('[data-slot="list-header"]'))
    const coverBottom = headers.reduce((value, header) => {
      const headerRect = header.getBoundingClientRect()
      if (headerRect.top <= rect.top + 1 && headerRect.bottom > rect.top) return Math.max(value, headerRect.bottom)
      return value
    }, rect.top)
    return {
      items: new Set(
        Array.from(scroll.querySelectorAll<HTMLElement>('[data-slot="list-item"][data-key]'))
          .filter((node) => {
            const nodeRect = node.getBoundingClientRect()
            return nodeRect.top < coverBottom || nodeRect.bottom > rect.bottom
          })
      ...
      headers: new Set(
        headers
          .filter((node) => {
            if (node.getAttribute("data-stuck") === "true") return false
            const nodeRect = node.getBoundingClientRect()
  ...
  createEffect(
    on(
      active,
      (key) => {
        const all = untrack(flat)
        if (untrack(() => store.mouseActive) || all.length === 0) return
        const scroll = scrollRef()
        if (!scroll) return
  ...
  function GroupHeader(groupProps: { group: { category: string; items: T[] } }): JSX.Element {
    ...
    return (
      <div
        ...
        data-key={groupProps.group.category}
        data-stuck={state.stuck}
        classList={{ invisible: clipped().headers.has(groupProps.group.category) }}
  ...
  return (
    <div
      data-component="list"
      data-plain-scroll={props.plainScroll ? "true" : undefined}
    ...
    >
      <Show when={!!props.search}>
        <div data-slot="list-search-wrapper">
          ...
          <Show when={searchAction()}>
            <div data-slot="list-search-action" data-multi-action={searchProps().multiAction ? "true" : undefined}>
              {searchAction()}
            </div>
          </Show>
        </div>
      ...
      <div
        ...
        onMouseLeave={() => setStore("mouseActive", false)}
        onScroll={(event) => {
          setStore("scrollTop", event.currentTarget.scrollTop)
          if (store.mouseActive) setActive(null)
        }}
      >
        <Show
                      ...
                      {(item, i) => {
                        const node = (
                          <button
                            ...
                            data-active={!store.mouseActive && props.key(item) === active()}
                            data-selected={item === props.current}
                            classList={{ invisible: clipped().items.has(props.key(item)) }}
                          ...
                          >
                            <span data-slot="list-item-content">{props.children(item)}</span>
