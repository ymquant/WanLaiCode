...
const diffKeys = ["fileDiff", "before", "after", "virtual", ...sharedKeys] as const
...
function DiffViewer<T>(props: DiffFileProps<T>) {
  ...
  const sharedVirtuals = createSharedVirtualStrategy(() => viewer.container)
  const localVirtuals = createLocalVirtualStrategy(() => viewer.wrapper, () => local.virtual === "local")
  ...
  createEffect(() => {
    ...
    const virtualizer =
      local.virtual === false
        ? (sharedVirtuals.cleanup(), localVirtuals.cleanup(), undefined)
        : local.virtual === "local"
          ? (sharedVirtuals.cleanup(), localVirtuals.get())
          : (localVirtuals.cleanup(), sharedVirtuals.get())
  ...
  onCleanup(() => {
    instance?.cleanUp()
    instance = undefined
    sharedVirtuals.cleanup()
    localVirtuals.cleanup()
    dragSide = undefined
    dragEndSide = undefined
  })
