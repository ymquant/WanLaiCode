...
export function DiffChanges(props: {
  ...
  // 大批量文件列表只需要稳定展示终值；关闭逐位动画可避免每个数字创建 30 个轨道节点。
  animated?: boolean
  // 运行中：即使当前增删为 0 也保持渲染（占位脉冲），并让计数器在完成时从 0 滚到终值
  pending?: boolean
}) {
  ...
  return (
    <Show when={variant() === "default" ? props.pending || total() > 0 : true}>
      <div
        ...
        <Switch>
          ...
          <Match when={variant() === "default"}>
            <span data-slot="diff-changes-additions">
              +
              <Show when={props.animated !== false} fallback={<span>{additions() ?? 0}</span>}>
                <AnimatedNumber value={additions() ?? 0} />
              </Show>
            </span>
            <span data-slot="diff-changes-deletions">
              -
              <Show when={props.animated !== false} fallback={<span>{deletions() ?? 0}</span>}>
                <AnimatedNumber value={deletions() ?? 0} />
              </Show>
            </span>
          </Match>
        </Switch>
