...
import defaultSprite from "./file-icons/sprite.svg"
import easySprite from "./file-icons-easy/sprite.svg"
import type { IconName as DefaultIconName } from "./file-icons/types"
import type { IconName as EasyIconName } from "./file-icons-easy/types"
...
type IconTheme = "default" | "easy"

// Code-level switch for the file icon theme. Not exposed in the UI.
const ACTIVE_ICON_THEME: IconTheme = "easy"

type IconName = DefaultIconName | EasyIconName
type IconChoice = { name: IconName; theme: IconTheme }

export const FileIcon: Component<FileIconProps> = (props) => {
  const [local, rest] = splitProps(props, ["node", "class", "classList", "expanded", "mono"])
  const choice = createMemo(() => chooseIconName(local.node.path, local.node.type, local.expanded || false))
  const sprite = createMemo(() => (choice().theme === "easy" ? easySprite : defaultSprite))
  const name = createMemo(() => choice().name)
...
const DEFAULT_ICON_MAPS: IconMaps = {
  fileNames: {
    // Documentation files
    "readme.md": "Readme",
...
const EASY_ICON_MAPS: IconMaps = {
  fileNames: {
    ".gitignore": "Gitcat",
    ".gitattributes": "Gitcat",
    ".gitkeep": "Gitcat",
    ".gitmodules": "Gitcat",
  },
  fileExtensions: {
...
function lookupDirectory(maps: IconMaps, baseLower: string, expanded: boolean): IconName | undefined {
  for (const cand of folderNameVariants(baseLower)) {
    const icon = maps.folderNames[cand]
    if (icon) return expanded ? toOpenVariant(icon) : icon
  }
  return undefined
}

function lookupFile(maps: IconMaps, baseLower: string): IconName | undefined {
  const byName = maps.fileNames[baseLower]
  if (byName) return byName

  for (const ext of dottedSuffixesDesc(baseLower)) {
    const icon = maps.fileExtensions[ext]
    if (icon) return icon
  }
  return undefined
}

export function chooseIconName(path: string, type: "directory" | "file", expanded: boolean): IconChoice {
  const base = basenameOf(path)
  const baseLower = base.toLowerCase()

  if (type === "directory") {
    if (ACTIVE_ICON_THEME === "easy") {
      // Easy theme has only flat Folder/FolderOpen icons; use them consistently
      // for all directories instead of mixing in default theme's named folders.
      return { name: expanded ? EASY_ICON_MAPS.defaults.folderOpen : EASY_ICON_MAPS.defaults.folder, theme: "easy" }
    }
    const fallback = lookupDirectory(DEFAULT_ICON_MAPS, baseLower, expanded)
    if (fallback) return { name: fallback, theme: "default" }
    return { name: expanded ? DEFAULT_ICON_MAPS.defaults.folderOpen : DEFAULT_ICON_MAPS.defaults.folder, theme: "default" }
  }

  if (ACTIVE_ICON_THEME === "easy") {
    const easy = lookupFile(EASY_ICON_MAPS, baseLower)
    if (easy) return { name: easy, theme: "easy" }
  }

  const fallback = lookupFile(DEFAULT_ICON_MAPS, baseLower)
  if (fallback) return { name: fallback, theme: "default" }

  if (ACTIVE_ICON_THEME === "easy") {
    return { name: EASY_ICON_MAPS.defaults.file, theme: "easy" }
  }
  return { name: DEFAULT_ICON_MAPS.defaults.file, theme: "default" }
}
