...
import { prewarmPromptDraft, resetPromptDraft } from "@/context/prompt"
import { messageAgentColor } from "@/utils/agent"
import { sessionTitle } from "@/utils/session-title"
import { resolvedSessionStatusBusy, sessionHasRunningTurn } from "../session/followup-queue"
...
export const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
  ...
  const hasPermissions = createMemo(() =>
    dirs().some((directory) => {
      ...
      return hasProjectPermissions(store.permission)
    }),
  )
...
export const SessionItem = (props: SessionItemProps): JSX.Element => {
  ...
  const isWorking = createMemo(() => {
    ...
    // 侧边栏的转圈要跟会话页同一套语义运行态走，避免失败回合已落错误内容但 status 残留时一直转。
    return sessionHasRunningTurn({
      messages: sessionStore.message[props.session.id],
      partsByMessage: sessionStore.part,
      // 稀疏 status map 在 ready 后缺失即 idle；首次快照前仍保留未知，避免误判真实长回合。
      statusBusy: resolvedSessionStatusBusy({
        status,
        snapshotReady: sessionStore.session_status_ready,
        sessionKnown: sessionStore.session_status_known[props.session.id],
      }),
      // retry 是服务端确认的活动 runner，不能被失败 attempt 的完成证据覆盖成空闲。
  ...
  createEffect(() => {
    ...
    // 侧边栏也需要被时间宽限唤醒，否则会话页恢复后列表仍可能继续转圈。
  ...
  const warm = (span: number, priority: "high" | "low") => {
    ...
    // 同时预热该会话的持久化草稿读取，打开时输入框不用再等异步 IPC
