...
describe("layout deep links", () => {
  ...
  test("parses registry plugin install deep links", () => {
    expect(parsePluginInstallDeepLink("wanlaicode://plugin/install?namespace=acme&slug=demo&version=1.2.3")).toEqual({
      namespace: "acme",
      slug: "demo",
      version: "1.2.3",
    })
    expect(parsePluginInstallDeepLink("opencode://plugin/install?namespace=acme&slug=demo")).toEqual({
      namespace: "acme",
      slug: "demo",
    })
  })

  test("ignores invalid registry plugin install deep links", () => {
    expect(parsePluginInstallDeepLink("wanlaicode://plugin/install?namespace=acme")).toBeUndefined()
    expect(parsePluginInstallDeepLink("wanlaicode://plugin/preview?namespace=acme&slug=demo")).toBeUndefined()
    expect(
      parsePluginInstallDeepLink("wanlaicode://plugin/install?url=https%3A%2F%2Fexample.com%2Fdemo.tgz"),
    ).toBeUndefined()
    expect(parsePluginInstallDeepLink("wanlaicode://plugin/install?namespace=../acme&slug=demo")).toBeUndefined()
    expect(
      parsePluginInstallDeepLink("wanlaicode://plugin/install?namespace=acme&slug=demo&version=bad%20version"),
    ).toBeUndefined()
  })

  test("collects only valid registry plugin install deep links", () => {
    expect(
      collectPluginInstallDeepLinks([
        "wanlaicode://plugin/install?namespace=acme&slug=one",
        "wanlaicode://open-project?directory=/tmp/demo",
        "wanlaicode://plugin/install?namespace=acme&slug=two&version=2.0.0",
      ]),
    ).toEqual([
      { namespace: "acme", slug: "one" },
      { namespace: "acme", slug: "two", version: "2.0.0" },
    ])
  })
  ...
  test("drains global deep links once", () => {
    const target = {
      __WANLAICODE__: {
        deepLinks: ["wanlaicode://open-project?directory=/a"],
      },
    } as unknown as Window & { __WANLAICODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["wanlaicode://open-project?directory=/a"])
    expect(drainPendingDeepLinks(target)).toEqual([])
  })

  test("drains legacy opencode deep links", () => {
    const target = {
      __OPENCODE__: {
    ...
    expect(drainPendingDeepLinks(target)).toEqual(["opencode://open-project?directory=/legacy"])
    expect(drainPendingDeepLinks(target)).toEqual([])
  })
})
