...
import { FileOpenErrorPlaceholder } from "@/components/file-preview/binary-placeholder"
...
export function FileTabContent(props: { tab: string }) {
  ...
  const skillPreview = createMemo(() => getSkillPreview(path()))
  ...
  const [note, setNote] = createStore({
    ...
    commenting: {} as Record<string, SelectedLineRange>,
  ...
  const commentsUi = createLineCommentController({
    ...
    state: {
      ...
      commentingRanges: () =>
        Object.entries(note.commenting).map(([key, range]) => ({ key, range })),
      addCommenting: (key, range) => setNote("commenting", key, range),
      removeCommenting: (key) =>
        setNote("commenting", (prev) => {
          const next = { ...prev }
          delete next[key]
          return next
        }),
  ...
  const renderFile = (source: string) => (
    <div class="relative overflow-hidden pb-40">
      <Dynamic
        ...
        mode="text"
        file={{
          name: skillPreview()?.displayName ?? path() ?? "",
          contents: source,
          cacheKey: cacheKey(),
        }}
        enableLineSelection
