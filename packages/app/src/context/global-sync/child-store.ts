...
import { loadLspQuery, loadMcpQuery } from "../global-sync-queries"
import { agentsQueryKeyMatches, directoryKey, providerListWithFallback, type DirectoryKey } from "./utils"
        ...
        createRoot((dispose) => {
          ...
          const [pathQuery, mcpQuery, lspQuery, providerQuery] = useQueries(() => ({
            queries: [
              ...
              { ...loadMcpQuery(key, sdk), enabled: false },
              loadLspQuery(key, sdk),
              loadProvidersQuery(key, sdk, input.queryClient),
            ],
          }))
          ...
          const child = createStore<State>({
            ...
            icon: initialIcon,
            get provider_ready() {
              return !providerQuery.isLoading || input.global.provider().all.length > 0
            },
            get provider() {
              ...
              return (
                providerListWithFallback({
                  ...
                  previous: input.queryClient?.getQueryData<ProviderListResponse>([key, "providers"]),
            ...
            sessionTotal: 0,
            session_status: {},
            // 首次完整 status 快照到达前，缺失 key 仍表示未知，不能提前当成 idle。
            session_status_ready: false,
            // 单会话事件只在这里记录自己的权威性，避免一个 idle 事件误判其它尚未同步的运行会话。
            ...
            question: {},
            get mcp_ready() {
              return mcpQuery.data !== undefined
            },
            get mcp() {
          ...
          // agents query 无持久 Observer（composer 只订阅加载态），后台 revalidate 的
          // 新结果要手动回写 store，否则已 bootstrap 的目录切回时 Agent 列表停在首次快照。
          // query 由各调用方以原始目录串建 key，这里按归一化目录匹配（兼容 Windows/尾斜杠）
          if (input.queryClient) {
            const unsubAgents = input.queryClient.getQueryCache().subscribe((event) => {
              if (event.type !== "updated") return
              if (!agentsQueryKeyMatches(event.query.queryKey, key)) return
              const data = event.query.state.data as State["agent"] | undefined
              if (data) child[1]("agent", data)
            })
