...
// codex 侧边栏在预取管线组件（layout）之外，经模块级注册桥接 hover/按下预取
let prefetchBridge: ((session: Session, priority: "high" | "low") => void) | undefined
export function registerSessionPrefetch(fn: typeof prefetchBridge) {
  prefetchBridge = fn
}
export function requestSessionPrefetch(session: Session, priority: "high" | "low" = "high") {
  prefetchBridge?.(session, priority)
}
