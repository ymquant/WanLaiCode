...
import { readFileContentWithLocalFallback } from "./file/local-file-fallback"
import {
  approxBytes,
  evictContentLru,
...
export const { use: useFile, provider: FileProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const tree = createFileTreeStore({
      ...
      onError: (dir, message) => {
        // 仅根目录 "" 失败时弹 toast（根目录只有一个，不会轰炸）。
        // 子目录错误通过 tree.dirState 返回，由文件树 UI 内联显示。
        if (dir === "") {
          showToast({
            variant: "error",
    ...
    const setLoadError = (file: string, message: string) => {
      setStore(
        "file",
        file,
        produce((draft) => applyFileLoadError(draft, message)),
      )
      removeFileContentBytes(file)
      // 原始错误保留在文件状态中供诊断；用户界面只显示稳定文案，避免暴露本机绝对路径。
      showToast({
        variant: "error",
        icon: "circle-x",
        class: "file-open-error-toast",
    ...
    const load = (input: string, options?: { force?: boolean }) => {
      ...
      const promise = readFileContentWithLocalFallback({
        path: file,
        readWorkspace: () => sdk.client.file.read({ path: file }).then((x) => x.data),
        // 绝对路径表示该文件未被归一化到当前工作区；服务端拒绝后由桌面主进程只读加载。
        readLocal: platform.readLocalFile,
      })
        .then((content) => {
    ...
    const invalidator = createWatcherInvalidator({
      ...
      isDirError: (dir) => Boolean(tree.dirState(dir)?.error),
      refreshDir: (dir) => {
        void tree.listDir(dir, { force: true })
      },
    })
    const stop = sdk.event.listen((e) => invalidator.handle(e.details))
    ...
    return {
      ...
      tree: {
        ...
        isUserCollapsed: tree.isUserCollapsed,
        toggle(input: string) {
