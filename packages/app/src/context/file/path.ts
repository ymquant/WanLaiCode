...
export function createPathHelpers(scope: () => string) {
  const normalize = (input: string) => {
    ...
    const absoluteInput = path.startsWith("/") || /^[A-Za-z]:[\\/]/.test(path) || path.startsWith("\\\\")
    const stripsWorkspaceRoot =
      canonPath.startsWith(canonRoot) &&
      (canonRoot.endsWith("/") || canonPath === canonRoot || canonPath[canonRoot.length] === "/")
    if (stripsWorkspaceRoot) {
    ...
    if ((stripsWorkspaceRoot || !absoluteInput) && (path.startsWith("/") || path.startsWith("\\"))) {
      path = path.slice(1)
    }
