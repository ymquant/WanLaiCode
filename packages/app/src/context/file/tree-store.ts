...
type TreeStoreOptions = {
  ...
  onError: (dir: string, message: string) => void
}
...
export function createFileTreeStore(options: TreeStoreOptions) {
  ...
  const listDir = (input: string, opts?: { force?: boolean }) => {
    ...
    if (!opts?.force && (current?.loaded || current?.error)) return Promise.resolve()
    ...
    const promise = options
      ...
      .catch((e) => {
        ...
        if (!hadError) options.onError(dir, e.message)
      })
      .finally(() => {
        inflight.delete(dir)
      })
  ...
  const expandDir = (input: string) => {
    ...
    ensureDir(dir)
    const current = tree.dir[dir]
    userCollapsed.delete(dir)
    setTree("dir", dir, "expanded", true)
    void listDir(dir, current?.error ? { force: true } : undefined)
  }

  const collapseDir = (input: string) => {
    const dir = options.normalizeDir(input)
    ensureDir(dir)
    userCollapsed.add(dir)
  ...
  return {
    ...
    isUserCollapsed: (path: string) => userCollapsed.has(path),
    reset,
  }
}
