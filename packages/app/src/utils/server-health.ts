...
function withLoopbackAlias(server: ServerConnection.HttpBase) {
  try {
...
async function healthFor(server: ServerConnection.HttpBase, fetch: typeof globalThis.fetch, signal?: AbortSignal) {
...
export async function checkServerHealth(
  ...
  const run = (current: ServerConnection.HttpBase) =>
    healthFor(current, fetch, signal).then((x) =>
      x.error ? Promise.reject(x.error) : { healthy: x.data?.healthy === true, version: x.data?.version },
    )
  ...
  const attempt = (count: number): Promise<ServerHealth> =>
    run(server).catch((error) => {
      const alias = count === 0 ? withLoopbackAlias(server) : undefined
      if (!alias) return next(count, error)
      return run(alias).catch(() => next(count, error))
    })
