...
describe("getSessionContextMetrics", () => {
  ...
  test("uses active model's limit when provided so usage% reflects the next request's target", () => {
    const messages = [assistant("a1", { input: 300, output: 0, reasoning: 0, read: 0, write: 0 }, 0, "p-old", "m-old")]
    const providers = [
      {
        id: "p-old",
        name: "OldProvider",
        models: { "m-old": { name: "Old Model", limit: { context: 1_000_000 } } },
      },
      {
        id: "p-new",
        name: "NewProvider",
        models: { "m-new": { name: "New Model", limit: { context: 1000 } } },
      },
    ]

    const metrics = getSessionContextMetrics(messages, providers, { providerID: "p-new", modelID: "m-new" })

    expect(metrics.context?.total).toBe(300)
    expect(metrics.context?.limit).toBe(1000)
    expect(metrics.context?.usage).toBe(30)
    expect(metrics.context?.providerLabel).toBe("NewProvider")
    expect(metrics.context?.modelLabel).toBe("New Model")
  })

  test("falls back to the last assistant message's model when active model is omitted", () => {
    const messages = [assistant("a1", { input: 250, output: 0, reasoning: 0, read: 0, write: 0 }, 0)]
    const providers = [{ id: "openai", name: "OpenAI", models: { "gpt-4.1": { name: "GPT-4.1", limit: { context: 1000 } } } }]
