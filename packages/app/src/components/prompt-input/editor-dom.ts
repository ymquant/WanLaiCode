...
export function getNodeLength(node: Node): number {
  ...
  if (node.nodeType === Node.ELEMENT_NODE && (node as HTMLElement).dataset.type === "skill") {
    return ((node as HTMLElement).dataset.content ?? node.textContent ?? "").replace(/\u200B/g, "").length
  }
  ...
  while (node) {
    ...
    const isPill =
      ...
      ((node as HTMLElement).dataset.type === "file" ||
        (node as HTMLElement).dataset.type === "file-reference" ||
        (node as HTMLElement).dataset.type === "agent" ||
        (node as HTMLElement).dataset.type === "link" ||
        (node as HTMLElement).dataset.type === "plugin" ||
        (node as HTMLElement).dataset.type === "skill" ||
        (node as HTMLElement).dataset.type === "conversation")
