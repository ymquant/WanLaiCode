...
describe("prompt-input history", () => {
  ...
  test("keeps different conversation references as separate history entries", () => {
    const conversation = (id: string, transcript: string): Prompt => [
      {
        type: "conversation",
        id,
        title: "聊天",
    ...
    const first = prependHistoryEntry([], conversation("chat_1", "User: one"))

    expect(prependHistoryEntry(first, conversation("chat_2", "User: two"))).toHaveLength(2)
    expect(prependHistoryEntry(first, conversation("chat_1", "User: updated"))).toHaveLength(2)
    expect(prependHistoryEntry(first, conversation("chat_1", "User: one"))).toBe(first)
  })
