...
function isPromptEqual(promptA: PromptHistoryStoredEntry, promptB: PromptHistoryStoredEntry) {
  ...
  for (let i = 0; i < entryA.prompt.length; i++) {
    ...
    if (partA.type === "link") {
      const b = partB.type === "link" ? partB : undefined
      if (!b || partA.content !== b.content || partA.href !== b.href || partA.plain !== b.plain) return false
    }
    if (partA.type === "file-reference") {
      const b = partB.type === "file-reference" ? partB : undefined
      if (!b || partA.content !== b.content || partA.href !== b.href) return false
    }
    if (partA.type === "file") {
    ...
    if (partA.type === "plugin") {
      const b = partB.type === "plugin" ? partB : undefined
      if (!b || partA.name !== b.name || partA.addonKey !== b.addonKey) return false
    }
    if (partA.type === "conversation") {
      const b = partB.type === "conversation" ? partB : undefined
      if (!b || partA.id !== b.id || partA.title !== b.title || partA.transcript !== b.transcript) return false
    }
