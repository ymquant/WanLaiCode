  ...
  const messages = createMemo((): ForkableMessage[] => {
    ...
    for (const message of msgs) {
      ...
      let preview = restored.prompt
        ...
        .replace(/\s+/g, " ")
        .trim()
      if (restored.addToChatSnippets.length > 0 && !preview) {
        preview = language.t(
          restored.addToChatSnippets.length === 1
            ? "session.addToChat.selectionCount.one"
            : "session.addToChat.selectionCount.other",
          { count: restored.addToChatSnippets.length },
        )
      }
  ...
  const handleSelect = (item: ForkableMessage | undefined) => {
    ...
    sdk.client.session
      ...
      .then((forked) => {
        ...
        dialog.close()
        prompt.set(restored.prompt, undefined, { dir, id: forked.data.id })
        prompt.addToChat.replace(restored.addToChatSnippets, { dir, id: forked.data.id })
        navigate(`/${dir}/session/${forked.data.id}`)
      })
      .catch((err: unknown) => {
        const dialogForkResolved = resolveError(err)
        showToast({
          title: language.t("common.requestFailed"),
          description: dialogForkResolved.category !== "unknown"
            ? language.t(dialogForkResolved.messageKey as any)
