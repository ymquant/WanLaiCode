...
import { PERF_REPORT_EVENT, type PerfReport } from "@/utils/perf-meter"
...
export function DebugBar() {
  ...
  const [prefs, setPrefs] = persisted(
    ...
    createStore<{ hidden: boolean; corner: Corner }>({ hidden: false, corner: "br" }),
  )
  const cycleCorner = () => {
    const i = CORNER_ORDER.indexOf(prefs.corner)
    setPrefs("corner", CORNER_ORDER[(i + 1) % CORNER_ORDER.length] ?? "br")
  }
  const [state, setState] = createStore({
    ...
    perf: {} as Record<string, PerfReport | undefined>,
  })
  // 性能面板隐藏时必须彻底停掉逐帧采样；空实现覆盖 onMount 前的首次渲染，挂载后再接入真实控制器。
  let startMonitoring = () => {}
  let stopMonitoring = () => {}
  ...
  const perf = (scope: string, key: string) => {
    const value = state.perf[scope]?.[key]
    if (typeof value !== "number") return
    return value
  }
  const streamQueue = () => perf("dialogue-stream", "queue_depth_max")
  const streamFlush = () => perf("dialogue-stream", "flush_ms_max")
  const streamReceived = () => perf("dialogue-stream", "received_count")
  const streamCoalesced = () => perf("dialogue-stream", "coalesced_count")
  const termPending = () => perf("terminal-output", "pending_chars_max")
  const termWrite = () => perf("terminal-output", "write_ms_max")
  const termMessages = () => perf("terminal-output", "ws_message_count")
  const termChars = () => perf("terminal-output", "ws_chars_count")
  const streamv = () => `${time(streamQueue()) ?? na()}/${time(streamFlush()) ?? na()}`
  const termv = () => `${time(termPending()) ?? na()}/${time(termWrite()) ?? na()}`
  ...
  onMount(() => {
    makeEventListener(window, PERF_REPORT_EVENT, (event) => {
      const detail = (event as CustomEvent<{ scope: string; row: PerfReport }>).detail
      if (!detail) return
      setState("perf", detail.scope, detail.row)
    })
    ...
    const vis = () => {
      if (document.visibilityState !== "visible" || prefs.hidden) {
        stop()
        return
      }
      resume()
    ...
    syncHeap()
    // 调试面板默认隐藏时不再常驻 requestAnimationFrame；这条循环会让 renderer/GPU 永久保持活跃。
    if (!prefs.hidden) start()
  ...
  return (
    <Show
      when={!prefs.hidden}
      fallback={
        <Tooltip value={language.t("debugBar.open")} placement="top">
          <button
            type="button"
            aria-label={language.t("debugBar.open")}
            class={`pointer-events-auto fixed z-50 size-6 rounded-full border border-border-base bg-surface-raised-stronger-non-alpha text-text-weak shadow-[var(--shadow-lg-border-base)] flex items-center justify-center hover:text-text-strong ${CORNER_CLASS[prefs.corner]}`}
            onClick={() => {
              setPrefs("hidden", false)
              startMonitoring()
            }}
          >
    ...
    >
      <aside
        ...
        // 性能浮层可能贴近 composer 右下角；整体透传鼠标事件，避免挡住发送/停止按钮。
        // 仅右上角控制按钮恢复 pointer-events，保留移动和关闭能力。
        class={`pointer-events-none fixed z-50 w-[308px] max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-xl border border-border-base bg-surface-raised-stronger-non-alpha p-0.5 text-text-strong shadow-[var(--shadow-lg-border-base)] sm:w-[324px] ${CORNER_CLASS[prefs.corner]}`}
      >
        <div class="pointer-events-auto absolute top-1 right-1 z-10 flex items-center gap-0.5">
          <Tooltip value={language.t("debugBar.move")} placement="top">
            <button
              type="button"
              aria-label={language.t("debugBar.move")}
              class="size-5 rounded-md flex items-center justify-center text-text-weak hover:text-text-strong hover:bg-surface-base"
              onClick={cycleCorner}
            >
              <Icon name="layout-left" size="small" />
            </button>
          </Tooltip>
          <Tooltip value={language.t("debugBar.close")} placement="top">
            <button
              type="button"
              aria-label={language.t("debugBar.close")}
              class="size-5 rounded-md flex items-center justify-center text-text-weak hover:text-text-strong hover:bg-surface-base"
              onClick={() => {
                setPrefs("hidden", true)
                stopMonitoring()
              }}
            >
        ...
        <div class="grid grid-cols-5 gap-px font-mono">
          ...
          <Cell
            ...
            wide
          />
          <Show when={state.perf["dialogue-stream"]}>
            <Cell
              label="STRM"
              tip={`dialogue stream queue/flush. received ${time(streamReceived()) ?? na()}, coalesced ${
                time(streamCoalesced()) ?? na()
              }`}
              value={streamv()}
              bad={bad(streamQueue(), 200) || bad(streamFlush(), 16)}
              dim={streamQueue() === undefined && streamFlush() === undefined}
            />
          </Show>
          <Show when={state.perf["terminal-output"]}>
            <Cell
              label="TERM"
              tip={`terminal pending/write. messages ${time(termMessages()) ?? na()}, chars ${time(termChars()) ?? na()}`}
              value={termv()}
              bad={bad(termPending(), 10_000) || bad(termWrite(), 16)}
              dim={termPending() === undefined && termWrite() === undefined}
            />
          </Show>
        </div>
      </aside>
