...
function createSessionEntries(props: {
  ...
  const sessions = (text: string, opts?: { includeEmpty?: boolean }) => {
    const query = text.trim()
    if (!query && !opts?.includeEmpty) {
      state.token += 1
...
function SessionEntryRow(props: { item: Entry; language: ReturnType<typeof useLanguage> }) {
  return (
...
export function DialogSelectFile(props: { mode?: DialogSelectFileMode; onOpenFile?: (path: string) => void }) {
  if (props.mode === "sessions") return <DialogSelectSession />
  return <DialogSelectFileAndCommand mode={props.mode} onOpenFile={props.onOpenFile} />
}

function DialogSelectSession() {
  ...
  const items = (text: string) => sessions(text, { includeEmpty: true })
  ...
  return (
    <Dialog class="pt-3 pb-0 !max-h-[480px]" transition>
      <List
        search={{
          placeholder: language.t("command.session.list.description"),
          autofocus: true,
          hideIcon: true,
        }}
      ...
      >
        {(item) => <SessionEntryRow item={item} language={language} />}
      </List>
    </Dialog>
  )
}

function DialogSelectFileAndCommand(props: {
  mode?: Exclude<DialogSelectFileMode, "sessions">
  ...
  const commandEntries = createCommandEntries({ hidden: filesOnly, command, language })
  ...
  const items = async (text: string) => {
    ...
    if (!query && filesOnly()) {
      ...
      const pending = state?.loaded ? Promise.resolve() : file.tree.list("")
      const next = uniqueEntries([...fileEntries.recent(), ...fileEntries.root()])

      if (state?.loaded || next.length > 0) {
        void pending
        return next
      }
        ...
        {(item) => (
          ...
          >
            ...
            <Match when={item.type === "session"}>
              <SessionEntryRow item={item} language={language} />
            </Match>
          </Switch>
        )}
      </List>
