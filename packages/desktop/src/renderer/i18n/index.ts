...
export function t(key: keyof Dictionary, params?: Record<string, string | number>) {
  // Inject the brand-aware appName so values can use the {{appName}} placeholder
  // without every caller having to thread it through.
  const merged = { appName: brandNameForLocale(state.locale), ...(params ?? {}) }
  return translate(key, merged)
}
