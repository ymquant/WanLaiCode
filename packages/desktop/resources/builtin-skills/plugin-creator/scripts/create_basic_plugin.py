...
def _xdg_data_home() -> Path:
    """Resolve XDG_DATA_HOME the same way the wanlaicode backend does.

    The backend computes its data dir as `$XDG_DATA_HOME/wanlaicode` (see
    packages/core/src/global.ts). Desktop sets XDG_DATA_HOME to the app userData
    dir before loading core, so we must read the env var here too — never hardcode
    `~/.local/share` — to stay aligned across Desktop and CLI.
    """
...
# 默认个人 marketplace root = `$XDG_DATA_HOME/wanlaicode/personal`,与后端 Global.Path.data
# 下的 personal 对齐;后端会自动加载该目录的 marketplace.json,无需手动 `marketplace add`。
DEFAULT_LOCAL_MARKETPLACE_ROOT = _xdg_data_home() / "wanlaicode" / "personal"
DEFAULT_PLUGIN_PARENT = DEFAULT_LOCAL_MARKETPLACE_ROOT / "plugins"
DEFAULT_MARKETPLACE_PATH = DEFAULT_LOCAL_MARKETPLACE_ROOT / ".agents" / "plugins" / "marketplace.json"
...
def build_plugin_json(plugin_name: str, *, with_mcp: bool, with_apps: bool) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "interface": {
            ...
            "shortDescription": f"Use {display_name} in wanlaicode.",
            "longDescription": f"{display_name} adds a local wanlaicode plugin scaffold.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Parent directory for plugin creation (defaults to "
            "$XDG_DATA_HOME/wanlaicode/personal/plugins, i.e. "
            "~/.local/share/wanlaicode/personal/plugins when XDG_DATA_HOME is unset). "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Create or update $XDG_DATA_HOME/wanlaicode/personal/.agents/plugins/marketplace.json "
            "by default (~/.local/share/... when XDG_DATA_HOME is unset). Marketplace entries always "
            "point to ./plugins/<plugin-name> relative to the marketplace root."
        ),
    )
    parser.add_argument(
        "--marketplace-path",
        ...
        help=(
            "Path to marketplace.json (defaults to "
            "$XDG_DATA_HOME/wanlaicode/personal/.agents/plugins/marketplace.json, "
...
def main() -> None:
    ...
    plugin_json_path = plugin_root / ".wanlaicode-plugin" / "plugin.json"
    write_json(
        plugin_json_path,
    ...
    if marketplace_path is not None:
        ...
        is_default_personal = (
            marketplace_path.resolve() == DEFAULT_MARKETPLACE_PATH.resolve()
        )
        if is_default_personal:
            print(
                f"\nNext step: the default personal marketplace is auto-loaded by wanlaicode, "
                f"so no `marketplace add` is needed.\n"
                f"Open the wanlaicode plugins page, refresh it, and you'll find '{plugin_name}' "
                f"under the 'Personal' category — click Install there."
            )
        else:
            print(
                f"\nNext step: register this non-default marketplace with wanlaicode before installing:\n"
                f"  wanlaicode addon marketplace add {marketplace_root}\n"
                f"Then open the wanlaicode plugins page and click Install on '{plugin_name}'."
            )
