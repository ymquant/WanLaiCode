...
const proxyFetch = NetProxy.create("Cli.import")
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, projectID: string) {
  ...
  const storedMessages = Database.use((db) =>
    db
      .select({ id: MessageTable.id, sequence: MessageTable.sequence })
      .from(MessageTable)
      .where(eq(MessageTable.session_id, row.id))
      .all(),
  )
  const storedSequences = new Map(storedMessages.map((message) => [message.id, message.sequence]))
  let nextSequence = storedMessages.reduce((max, message) => Math.max(max, message.sequence), -1) + 1

  for (const msg of exportData.messages) {
    const msgInfo = decodeMessageInfo(msg.info) as MessageV2.Info
    const { id, sessionID: _, ...msgData } = msgInfo
    // 重复导入保留同 ID 的原位置；新消息从当前末尾追加，避免与 (session_id, sequence) 唯一索引冲突。
    const sequence = storedSequences.get(id) ?? nextSequence++
    Database.use((db) =>
      db
        .insert(MessageTable)
        .values({
          id,
          session_id: row.id,
          // 导出消息数组已经是源会话顺序；导入时直接固化为 first-seen sequence。
          sequence,
