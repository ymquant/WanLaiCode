...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from wanlaicode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const wanlaicode = await wanlaicodeFiles(input)
  for (const file of wanlaicode) {
...
async function wanlaicodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "wanlaicode"),
    ...(await Filesystem.findUp(["wanlaicode.json", "wanlaicode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "wanlaicode"))
  }
  if (Flag.WANLAICODE_CONFIG) files.push(Flag.WANLAICODE_CONFIG)
