...
import wanlaicode from "./theme/wanlaicode.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "wanlaicode")
        draft.active = typeof active === "string" ? active : "wanlaicode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.wanlaicode, store.mode)
    })
