...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    event.subscribe((event) => {
      switch (event.type) {
        ...
        case "message.updated": {
          ...
          // 官方 turn.items 按事件首次到达顺序追加；同 ID 的流式更新只替换原位置，不能再按 ID 二分插入。
        ...
        case "message.removed": {
          ...
          if (!messages) break
          // message 列表按 first-seen 排列，删除也必须真实定位，避免逆 ID 顺序时漏删。
    ...
    const result = {
      ...
      get ready() {
        if ((process.env.WANLAICODE_FAST_BOOT ?? process.env.OPENCODE_FAST_BOOT)) return true
