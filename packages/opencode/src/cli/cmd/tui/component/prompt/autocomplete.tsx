...
export function Autocomplete(props: {
  ...
  const [mcpResourceWorkspace, setMcpResourceWorkspace] = createSignal<string>()

  createEffect(() => {
    if (store.visible !== "@") return
    const workspace = project.workspace.current()
    const key = workspace ?? ""
    if (mcpResourceWorkspace() === key) return
    setMcpResourceWorkspace(key)
    void sdk.client.experimental.resource.list({ workspace }).then((x) => sync.set("mcp_resource", x.data ?? {}))
  })
