...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".wanlaicode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".wanlaicode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".wanlaicode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.WANLAICODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.WANLAICODE_PURE && config.plugin_origins?.length) {
