...
export function Session() {
  ...
  const messagesBefore = (messageID?: string) => {
    if (!messageID) return messages()
    const cutoff = messages().findIndex((message) => message.id === messageID)
    // 已加载边界严格按 first-seen 位置裁剪；边界不在当前页时保留旧 ID 规则兼容历史会话。
    if (cutoff >= 0) return messages().slice(0, cutoff)
  ...
  const pending = createMemo(() => {
    const index = messages().findLastIndex((message) => message.role === "assistant" && !message.time.completed)
    // queued 标记比较时间线位置；-1 表示当前没有在途 assistant。
  ...
  command.register(() => [
    ...
    },
    {
      ...
      onSelect: async (dialog) => {
        ...
        const message = messagesBefore(session()?.revert?.messageID).findLast((item) => item.role === "user")
        if (!message) return
    ...
    },
    {
      ...
      onSelect: (dialog) => {
        ...
        if (!messageID) return
        const cutoff = messages().findIndex((message) => message.id === messageID)
        // redo 从边界后的真实数组位置找下一条用户消息，逆序 ID 不得改变恢复顺序。
        const message =
          cutoff >= 0
            ? messages()
                .slice(cutoff + 1)
                .find((item) => item.role === "user")
            : messages().find((item) => item.role === "user" && item.id > messageID)
        if (!message) {
    ...
    },
    {
      ...
      onSelect: (dialog) => {
        ...
        const lastAssistantMessage = messagesBefore(revertID).findLast((message) => message.role === "assistant")
  ...
  const revertRevertedMessages = createMemo(() => {
    ...
    if (!messageID) return []
    const cutoff = messages().findIndex((message) => message.id === messageID)
    // 回退摘要与实际清理共用位置水位；未加载旧边界才退回 ID 兼容判断。
    const reverted = cutoff >= 0 ? messages().slice(cutoff) : messages().filter((message) => message.id >= messageID)
    return reverted.filter((message) => message.role === "user")
  })
  ...
  const messageIsReverted = (message: Message, index: number) => {
    const messageID = revertMessageID()
    if (!messageID) return false
    // 当前页能定位水位时按数组索引隐藏；分页未包含水位时维持旧 ID 兜底。
    return revertMessageIndex() >= 0 ? index >= revertMessageIndex() : message.id >= messageID
  }
              ...
              <For each={messages()}>
                {(message, index) => (
                  <Switch>
                    ...
                    <Match when={messageIsReverted(message, index())}>
                      <></>
                    </Match>
...
function UserMessage(props: {
  ...
  const { theme } = useTheme()
  const [hover, setHover] = createSignal(false)
  // 父级传入在途 assistant 的数组位置；当前 user 位于其后才是尚未开始处理的队列项。
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.WANLAICODE_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
