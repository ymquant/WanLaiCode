...
const proxyFetch = NetProxy.create("Cli.mcp")
...
function configuredServers(mcpConfig: Config.Info["mcp"]) {
  return Object.entries(mcpConfig ?? {}).filter((entry): entry is [string, McpConfigured] => isMcpConfigured(entry[1]))
}

function oauthServers(mcpConfig: Config.Info["mcp"]) {
  return configuredServers(mcpConfig).filter(
...
function listState() {
  return Effect.gen(function* () {
    ...
    const mcpConfig = MCP.mergeMcpConfig(yield* addons.getAddons(), config.mcp)
    const statuses = yield* mcp.status()
    const stored = yield* Effect.all(
      Object.fromEntries(configuredServers(mcpConfig).map(([name]) => [name, mcp.hasStoredTokens(name)])),
      { concurrency: "unbounded" },
    )
    return { mcpConfig, statuses, stored }
  })
}
...
function authState() {
  return Effect.gen(function* () {
    ...
    const auth = yield* Effect.all(
      Object.fromEntries(oauthServers(mcpConfig).map(([name]) => [name, mcp.getAuthStatus(name)])),
      { concurrency: "unbounded" },
    )
    return { mcpConfig, auth }
  })
}
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    const { mcpConfig, statuses, stored } = yield* listState()
    const servers = configuredServers(mcpConfig)

    if (servers.length === 0) {
      prompts.log.warn("No MCP servers configured")
      prompts.outro("Add servers with: wanlaicode mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    const { mcpConfig, auth } = yield* authState()
    const mcpServers = mcpConfig
    const servers = oauthServers(mcpConfig)

    if (servers.length === 0) {
      prompts.log.warn("No OAuth-capable MCP servers configured")
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in wanlaicode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .wanlaicode/ subdirectory too)
  const candidates = [path.join(baseDir, "wanlaicode.json"), path.join(baseDir, "wanlaicode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".wanlaicode", "wanlaicode.json"), path.join(baseDir, ".wanlaicode", "wanlaicode.jsonc"))
  }
  ...
  // Default to wanlaicode.json if none exist
  return candidates[0]
}
