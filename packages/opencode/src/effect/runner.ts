...
export interface Runner<A, E = never> {
  ...
  // 等当前活动（run/shell）自然结束回到 Idle，不做中断。用于实例 dispose/reload 前
  // 先把在跑的回合排空，避免被动中断砍断生成。空闲则立即返回。
  ...
  const awaitIdle: Effect.Effect<void> = Effect.gen(function* () {
    while (true) {
      ...
      switch (st._tag) {
        ...
        case "Running":
        case "ShellThenRun":
          yield* Deferred.await(st.run.done).pipe(Effect.exit)
          break
        case "Shell":
          yield* Fiber.await(st.shell.fiber)
          break
      }
    }
  })
