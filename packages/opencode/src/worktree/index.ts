...
export const CreateInput = Schema.Struct({
  ...
  branchPrefix: Schema.optional(
    Schema.String.annotate({
      description: "Prefix added before the worktree's branch name (e.g. 'codex' produces codex/<name>). Defaults to 'wanlaicode'.",
    }),
  ),
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const DEFAULT_BRANCH_PREFIX = "wanlaicode"
    const normalizeBranchPrefix = (raw?: string) => {
      const value = (raw ?? "").trim().replace(/^\/+|\/+$/g, "")
      return value || DEFAULT_BRANCH_PREFIX
    }
    ...
    const setup = Effect.fnUntraced(function* (info: Info) {
      const ctx = yield* InstanceState.context
      const hasHead = yield* git(["rev-parse", "--verify", "--quiet", "HEAD"], { cwd: ctx.worktree })
      const addArgs =
        hasHead.code === 0
          ? ["worktree", "add", "--no-checkout", "-b", info.branch, info.directory]
          : ["worktree", "add", "--orphan", "-b", info.branch, info.directory]
      const created = yield* git(addArgs, { cwd: ctx.worktree })
      if (created.code !== 0) {
    ...
    const remove = Effect.fn("Worktree.remove")(function* (input: RemoveInput) {
      ...
      if (branch) {
        const branchRefCheck = yield* git(["show-ref", "--verify", "--quiet", `refs/heads/${branch}`], {
          cwd: ctx.worktree,
        })
        if (branchRefCheck.code === 0) {
