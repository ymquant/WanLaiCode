...
const proxyFetch = NetProxy.create("LSP.server")
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.WANLAICODE_DISABLE_LSP_DOWNLOAD) return
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.WANLAICODE_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!installed) {
      ...
      const download = await proxyFetch(releaseURL)
...
export const TerraformLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const releaseResponse = await proxyFetch("https://api.releases.hashicorp.com/v1/releases/terraform-ls/latest")
      if (!releaseResponse.ok) {
      ...
      const downloadResponse = await proxyFetch(build.url)
      if (!downloadResponse.ok) {
