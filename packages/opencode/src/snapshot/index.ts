...
const diffPatchFileLimit = 500
const diffPatchChangedLineLimit = 20_000
const diffPatchBlobByteLimit = 4 * 1024 * 1024
// core.fsyncObjectFiles=false 把 loose-object 写入的 fsync 跳过。
// snapshot 是 ephemeral 数据(crash 后 caller 总能从 worktree 重 track),
// 不需要 crash-safe 保证。在 QEMU 模拟 SCSI 控制器的 self-hosted runner 上
// fsync 单次 ~19ms,关掉之后 git add 280 文件从 ~5.7s 降到 ~0.5s (13×)。
// 现代 git 推荐的 core.fsync=-loose-object 在 git for windows 2.53 上行为
// 异常 (实测反而更慢),回到 legacy 选项,仍被 2.53 支持。
const core = ["-c", "core.longpaths=true", "-c", "core.symlinks=true", "-c", "core.fsyncObjectFiles=false"]
        ...
        const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
          return yield* locked(
            Effect.gen(function* () {
              ...
              const changedLines = rows.reduce((total, row) => total + row.additions + row.deletions, 0)
              if (rows.length > diffPatchFileLimit || changedLines > diffPatchChangedLineLimit) {
                // 超大工作区差异只保留文件和计数；必须在读取 blob 前返回，避免构造数百 MB 源码与 patch。
                log.info("snapshot diff exceeds patch count limit, returning metadata only", {
                  files: rows.length,
                  changedLines,
                })
                return metadata()
              ...
              const refs = refsForRows(rows)
              const sizes = refs.length
                ? yield* git(
                    [...cfg, ...args(["cat-file", "--batch-check=%(objectsize)"])],
                    {
                      cwd: state.directory,
                      stdin: Stream.make(new TextEncoder().encode(refs.map((item) => item.ref).join("\n") + "\n")),
                    },
                ...
                : undefined
              const blobBytes = sizes?.text
                .trim()
                .split("\n")
                .filter(Boolean)
                .reduce((total, value) => total + (Number.isFinite(Number(value)) ? Number(value) : 0), 0)
              if ((sizes && sizes.code !== 0) || (blobBytes ?? 0) > diffPatchBlobByteLimit) {
                // blob 预检失败时也选择元数据降级；绕过门禁回退 git show 会重新引入同一内存风险。
                log.info("snapshot diff exceeds patch blob limit, returning metadata only", {
                  files: rows.length,
                  blobBytes,
                  checkCode: sizes?.code,
                })
                return metadata()
              }
