  ...
  SessionEvent.All.match(event, {
    ...
    "session.next.step.ended": (event) => {
      if (currentAssistant) {
        adapter.updateAssistant(
          produce(currentAssistant, (draft) => {
            // Chat Completions 没有显式 phase；step 收尾时只补缺失值，Responses 已给出的阶段保持不变。
            const phase = ["tool-calls", "unknown"].includes(event.data.finish) ? "commentary" : "final_answer"
            draft.content.forEach((part) => {
              if (part.type === "text" && !part.phase) part.phase = phase
            })
            draft.time.completed = event.data.timestamp
        ...
        adapter.updateAssistant(
          produce(currentAssistant, (draft) => {
            ...
            if (match) {
              ...
              // 旧事件没有 phase 时保留 Started 已知值；Ended 的显式值才可以纠正阶段。
              if (event.data.phase) match.phase = event.data.phase
            }
          }),
        )
      }
          ...
          produce(currentAssistant, (draft) => {
            ...
            if (match && match.state.status === "running") {
              ...
              match.state = {
                ...
                input: match.state.input,
                attachments: event.data.content?.flatMap((item) => (item.type === "file" ? [item] : [])),
                structured: event.data.structured ?? match.state.structured,
                content: [...(event.data.content ?? match.state.content)],
              }
            }
          }),
        )
