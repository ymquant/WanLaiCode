  ...
  Effect.gen(function* () {
    ...
    const connectSSE = Effect.fn("Workspace.connectSSE")(function* (
      ...
      if (response.status < 200 || response.status >= 300) {
        // Drain the body so InterruptibleResponse's Stream.ensuring fires
        // controller.abort() and releases the underlying fetch socket. Without
        // this, an unread error response sits in the keep-alive pool until GC
        // runs the FinalizationRegistry callback, which on Bun/Linux under
        // load can lag long enough to block server.close() on cleanup.
    ...
    const startSync = Effect.fn("Workspace.startSync")(function* (space: Info) {
      if (!Flag.WANLAICODE_EXPERIMENTAL_WORKSPACES) return
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const workspaceAuth = Object.fromEntries(
        Object.entries(yield* auth.all()).filter(([key]) => !Auth.isRemoteControlCredentialKey(key)),
      )
      const env = {
        // 远控设备令牌只属于本机桌面控制面，绝不能复制给工作区子进程形成第二个同身份网关。
        OPENCODE_AUTH_CONTENT: JSON.stringify(workspaceAuth),
