...
export class OpenAIResponsesLanguageModel implements LanguageModelV3 {
          ...
          output: z.array(
            z.discriminatedUnion("type", [
              z.object({
                ...
                id: z.string(),
                // 非流式 Responses 与流式 item 使用同一 phase 契约，避免 generate 路径静默剥离。
  ...
  async doStream(options: LanguageModelV3CallOptions) {
    ...
    // Copilot 可能在不同事件里轮换 item_id；按 output_index 保存 added 的稳定 ID 与阶段，供 done 事件回退。
    const activeTextItems: Record<
      number,
      { id: string; phase: "commentary" | "final_answer" | undefined } | undefined
    > = {}
    // text delta 没有 output_index，仍需用最近 added 的稳定 ID 归并 Copilot 轮换后的 item_id。
          ...
          transform(chunk, controller) {
            ...
            if (isResponseOutputItemAddedChunk(value)) {
              if (value.item.type === "function_call") {
                ...
                // added 事件是该 output item 的起点，done 即使省略 phase 也必须沿用这里的官方值。
                const phase = value.item.phase ?? undefined
                activeTextItems[value.output_index] = { id: value.item.id, phase }
                ...
                controller.enqueue({
                  ...
                  providerMetadata: {
                    openai: {
                      ...
                      // Responses 的 phase 决定该消息留在活动流还是进入最终回复区，适配层必须原样透传。
                      ...(phase ? { phase } : {}),
                    },
                  },
                })
              ...
              if (value.item.type === "function_call") {
                ...
                // 少数网关会漏掉 added；此时继续沿用 delta 路径建立的稳定 ID，保持旧兼容行为。
                const activeText =
                  activeTextItems[value.output_index] ??
                  (currentTextId ? { id: currentTextId, phase: undefined } : undefined)
                if (activeText) {
                  // done 的显式 phase 可以纠正 added；缺省或 null 时保留 added 已知值。
                  const phase = value.item.phase ?? activeText.phase
                  controller.enqueue({
                    type: "text-end",
                    id: activeText.id,
                    providerMetadata: {
                      openai: {
                        // phase 采用 done 的权威值，但 itemId 必须沿用 added 的稳定值；Copilot 可能在完成事件轮换 ID。
                        itemId: activeText.id,
                        ...(phase ? { phase } : {}),
                      },
                    },
                  })
                  activeTextItems[value.output_index] = undefined
                  if (currentTextId === activeText.id) currentTextId = null
                }
              ...
              // openAIResponsesFailedSSEPayload)。标成 STREAM_FAILED「流中断」错误抛出
              // (processor 的 case "error" 会 throw value.error)→ fromError 映射为可重试 APIError,
              // 交给 SessionRetry 的流中断门控:首 token 前无感重试自愈;已产出内容后不重试、标红错误
              // (避免重试不回滚已写 part 造成内容重复/串台,对齐 PR #253 的不变量)。
              finishReason = { unified: "error", raw: value.response?.error?.code ?? undefined }
              const message = value.response?.error?.message ?? "Upstream stream failed before completion"
              controller.enqueue({
                type: "error",
                error: Object.assign(new Error(message), { code: "STREAM_FAILED" }),
              })
            }
          },
...
const responseOutputItemAddedSchema = z.object({
  ...
  item: z.discriminatedUnion("type", [
    z.object({
      type: z.literal("message"),
      id: z.string(),
      // 不声明会被 Zod strip，processor 就只能按 finishReason 猜测并破坏 commentary 顺序。
      phase: z.enum(["commentary", "final_answer"]).nullish(),
    }),
    z.object({
      type: z.literal("reasoning"),
...
const responseOutputItemDoneSchema = z.object({
  ...
  item: z.discriminatedUnion("type", [
    z.object({
      type: z.literal("message"),
      id: z.string(),
      // output_item.done 可能才携带最终 phase，必须和 added 事件使用同一 schema。
      phase: z.enum(["commentary", "final_answer"]).nullish(),
    }),
    z.object({
      type: z.literal("reasoning"),
