...
import { WanlaiCodeCredentialState } from "./wanlaicode-credential-state"
...
export const WanlaiCodeApiKeyValidateInput = Schema.Struct({
  apiKey: Schema.String,
  apiBase: Schema.optional(Schema.String),
}).pipe(withStatics((s) => ({ zod: zod(s) })))
export type WanlaiCodeApiKeyValidateInput = Schema.Schema.Type<typeof WanlaiCodeApiKeyValidateInput>

export const WanlaiCodeApiKeyValidateResult = Schema.Struct({
  ok: Schema.Literal(true),
}).pipe(withStatics((s) => ({ zod: zod(s) })))
export type WanlaiCodeApiKeyValidateResult = Schema.Schema.Type<typeof WanlaiCodeApiKeyValidateResult>
  ...
  Effect.gen(function* () {
    ...
    const callback = Effect.fn("ProviderAuth.callback")(function* (input: { providerID: ProviderID } & CallbackInput) {
      ...
      if ("refresh" in result) {
        ...
        // 实际桌面 OAuth callback 从 ProviderAuth 落盘；成功登录必须恢复本次凭据，即使服务端复用同一 revision。
        if (input.providerID === "wanlaicode") WanlaiCodeCredentialState.clearCredentialInvalid(info)
      }
    })
