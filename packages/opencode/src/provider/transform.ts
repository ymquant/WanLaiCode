...
export const OUTPUT_TOKEN_MAX = Flag.WANLAICODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
function reasoningOptionEfforts(model: Provider.Model) {
  return model.reasoning_options?.find((item) => item.type === "effort" && item.values?.length)?.values
}
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  switch (model.api.npm) {
    ...
    case "@ai-sdk/openai-compatible": {
      // wanlaicode 的档位能力由网关 /v1/models 下发。新版 reasoning_options 是权威字段，
      // 旧 reasoning_efforts 仅在没有新版字段时兜底；其它 openai-compatible provider 继续走启发式。
      const downstream = model.providerID === "wanlaicode" ? (reasoningOptionEfforts(model) ?? model.reasoning_efforts) : undefined
      if (model.providerID === "wanlaicode" && downstream && downstream.length > 0) {
        return Object.fromEntries(downstream.map((effort) => [effort, { reasoningEffort: effort }]))
      }
      const efforts = [...WIDELY_SUPPORTED_EFFORTS]
...
export function smallOptions(model: Provider.Model) {
  ...
  if (model.providerID === "wanlaicode") {
    // wanlaicode 走 @ai-sdk/openai-compatible。标题/建议/翻译这类后台小任务不需要思考；
    // 推理型模型若放任默认 reasoning，会把有限的 output 预算耗在思考上 → finish=length
    // 被丢弃，且开放式任务下延迟很高。把 reasoning 压到最低档（wanlai 的 deepseek-v4
    // 最低只支持 "low"，无 none/minimal），给实际输出留出预算。非推理模型无需处理。
    if (model.capabilities.reasoning) return { reasoningEffort: "low" }
    return {}
  }
