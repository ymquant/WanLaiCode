...
export type PatchDeps = {
  ...
  files: (dir: string, name: "wanlaicode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".wanlaicode")
}

function patchName(kind: Kind): "wanlaicode" | "tui" {
  if (kind === "server") return "wanlaicode"
  return "tui"
}
