...
const projectFiles = (config: Config.Info) => [
  ...(config.instruction_import?.agents_md !== false ? ["AGENTS.md"] : []),
  ...(!Flag.WANLAICODE_DISABLE_CLAUDE_CODE_PROMPT && config.instruction_import?.claude_md !== false
    ? ["CLAUDE.md"]
    : []),
  "CONTEXT.md",
]
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const systemPaths = Effect.fn("Instruction.systemPaths")(function* () {
      ...
      const paths = new Set<string>()
      const globalFiles = [
        ...(config.instruction_import?.agents_md !== false ? [path.join(global.config, "AGENTS.md")] : []),
        ...(!Flag.WANLAICODE_DISABLE_CLAUDE_CODE_PROMPT && config.instruction_import?.claude_md !== false
      ...
      if (!Flag.WANLAICODE_DISABLE_PROJECT_CONFIG) {
        ...
        for (const file of projectFiles(config)) {
          const filepath = path.join(root, file)
          if (yield* fs.existsSafe(filepath)) paths.add(path.resolve(filepath))
        }
      }
    ...
    const system = Effect.fn("Instruction.system")(function* () {
      ...
      return [
        ...
        ...(config.rules ?? []).flatMap((rule) =>
          rule.enabled && rule.title.trim() && rule.content.trim()
            ? [`Rule: ${rule.title.trim()}\n${rule.content.trim()}`]
            : [],
        ),
      ]
    })
    ...
    const resolve = Effect.fn("Instruction.resolve")(function* (
      ...
      while (current.startsWith(root) && current !== root) {
        const config = yield* cfg.get()
        for (const file of projectFiles(config)) {
          const found = path.resolve(path.join(current, file))
          if (found === target || sys.has(found) || already.has(found)) continue

          const exists = yield* fs.existsSafe(found)
          if (!exists) continue
