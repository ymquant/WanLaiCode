...
export const SessionTable = sqliteTable(
  "session",
  {
    ...
    forked_from: text({ mode: "json" }).$type<{ sessionID: SessionID; messageID?: MessageID }>(),
...
export const MessageTable = sqliteTable(
  "message",
  {
    ...
    // 持久化消息首次进入会话的顺序；同 ID 流式更新不改变该值，对齐官方 turn.items 的 push 语义。
    sequence: integer().notNull(),
    ...Timestamps,
    data: text({ mode: "json" }).notNull().$type<InfoData>(),
  },
  (table) => [
    uniqueIndex("message_session_sequence_idx").on(table.session_id, table.sequence),
