...
export function toPartialRow(info: DeepPartial<Session.Info>) {
  const obj = {
    ...
    // 远控模型切换通过 Session.setModel 产生增量事件；必须把 model 写入 session 表才能跨重启恢复。
    model: grab(info, "model"),
  ...
  SyncEvent.project(MessageV2.Event.Updated, (db, data) => {
    ...
    try {
      const current = db
        ...
        .get()
      // projector 位于 immediate 事务内；首次消息使用本会话 max+1，同 ID 更新沿用原 sequence。
      const sequence =
        current?.sequence ??
        ((db
          .select({ value: max(MessageTable.sequence) })
          .from(MessageTable)
      ...
      db.insert(MessageTable)
        ...
        // 因果时间修正也要覆盖索引列；否则已有 assistant 的 JSON 已更新，分页游标仍会沿用旧时间。
        .onConflictDoUpdate({ target: MessageTable.id, set: { time_created, data: rest } })
        .run()
    } catch (err) {
