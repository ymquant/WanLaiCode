...
export function parameterSchema(description: string) {
  return Schema.Struct({
    ...
    run_in_background: Schema.optional(Schema.Boolean).annotate({
      description:
        "When true, the command runs in the background and immediately returns a background id (e.g. bash_1). Use this for long-running tasks (servers, watchers, long downloads, backtests). Monitor progress with bash-output and stop it with kill-shell.",
    }),
...
2. Command Execution:
   ...
   - Examples of proper quoting:
     - mkdir "/Users/developer/My Documents" (correct)
     - mkdir /Users/developer/My Documents (incorrect - will fail)
...
Usage notes:
  ...
  - You can specify an optional timeout in milliseconds. If not specified, commands will time out after 120000ms (2 minutes). If the bash-output and kill-shell tools are available, a command that exceeds its timeout is moved to the background instead of being killed, and you get a background id (e.g. bash_1); use the bash-output tool to monitor it and kill-shell to stop it. Otherwise the command is stopped when it times out.
  - For long-running or blocking commands (dev servers, file watchers, long downloads, backtests), set run_in_background: true to start it in the background immediately and keep working; poll progress with bash-output and stop it with kill-shell.
