...
/**
 * 用户已中断（暂停目标 / ESC / 停止按钮）时跳过工具的收尾步骤，直接取 `aborted` 这个值。
 *
 * 写文件类工具落盘之后还要跑格式化与 LSP 诊断，这两步是秒级的：不跳过的话，用户按了停止
 * 还得干等它们跑完，界面上表现成「停不下来、还在编辑」。收尾只是锦上添花，文件此时已经
 * 完整落盘，跳过不会留下半个文件。每个收尾点各判一次而不是入口判一次，因为前一步本身可能就很慢。
 */
export function unlessAborted<A, E, R>(ctx: Context, effect: Effect.Effect<A, E, R>, aborted: A) {
  return ctx.abort.aborted ? Effect.succeed(aborted) : effect
}
