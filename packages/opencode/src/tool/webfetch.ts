    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          // 超时必须覆盖整个网络阶段：取响应头 + 读响应体。
          // 否则服务器秒回响应头、body 却卡住时，response.arrayBuffer 会无限挂起而超时不触发。
          ...
          // ctx.abort 让用户停止/ESC 能真正中断在途请求；超时兜底防止永久挂起。
          const { response, arrayBuffer } = yield* raceAbort(fetchEffect, ctx.abort).pipe(
