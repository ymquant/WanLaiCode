...
import { UpdateGoalTool, GetGoalTool } from "./goal"
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const automationcreate = yield* AutomationCreateTool
    const updategoal = yield* UpdateGoalTool
    const getgoal = yield* GetGoalTool
    const memory = yield* MemoryTool.pipe(Effect.provide(MemoryProcessor.defaultLayer))
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        for (const match of matches) {
          ...
          // 文件名对应默认导出的工具 ID；显式禁用时必须在 import 前跳过，避免执行无用的顶层代码。
          if (cfg.tools?.[namespace] === false) {
            log.debug("skipping disabled custom tool", { path: match, tool: namespace })
            continue
          }
          // 项目工具彼此隔离：单个文件缺依赖或顶层执行失败时保留日志，但不能阻断普通会话和其他工具。
          const mod = yield* Effect.tryPromise({
            try: () => import(pathToFileURL(match).href),
            catch: (error) => (error instanceof Error ? error : new Error(String(error))),
          }).pipe(
            Effect.catch((error) => {
              log.warn("failed to load custom tool, skipping", { path: match, error: error.message })
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.WANLAICODE_CLIENT) || Flag.WANLAICODE_ENABLE_QUESTION_TOOL
        ...
        const tool = yield* Effect.all({
          ...
          plan: Tool.init(plan),
          automationCreate: Tool.init(automationcreate),
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            tool.automationCreate,
            ...(Flag.WANLAICODE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.WANLAICODE_EXPERIMENTAL_PLAN_MODE && Flag.WANLAICODE_CLIENT === "cli" ? [tool.plan] : []),
            tool.updateGoal,
            tool.getGoal,
            tool.memory,
            tool.memoryRead,
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.opencode || Flag.WANLAICODE_ENABLE_EXA
        }
