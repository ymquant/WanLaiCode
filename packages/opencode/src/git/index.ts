...
/**
 * Windows: do not override `core.symlinks` (else `git status` lists far more paths than the shell) or
 * `core.autocrlf` (forcing `false` makes `git diff --numstat` treat CRLF↔LF as every line changed, so +/− explode).
 */
const cfg = [
  "--no-optional-locks",
  ...(process.platform === "win32" ? [] : (["-c", "core.autocrlf=false"] as const)),
  "-c",
...
const ws = (o?: { ignoreWhitespace?: boolean }) => (o?.ignoreWhitespace ? (["-w"] as const) : ([] as const))
...
export interface Interface {
  ...
  readonly diff: (cwd: string, ref: string, opts?: DiffOptions) => Effect.Effect<Item[]>
  readonly diffWorktree: (cwd: string, opts?: DiffOptions) => Effect.Effect<Item[]>
  readonly diffCached: (cwd: string, opts?: DiffOptions) => Effect.Effect<Item[]>
  readonly stats: (cwd: string, ref: string, opts?: DiffOptions) => Effect.Effect<Stat[]>
  readonly statsWorktree: (cwd: string, opts?: DiffOptions) => Effect.Effect<Stat[]>
  readonly statsCached: (cwd: string, opts?: DiffOptions) => Effect.Effect<Stat[]>
  readonly patch: (cwd: string, ref: string, file: string, options?: PatchOptions) => Effect.Effect<Patch>
  readonly patchWorktree: (cwd: string, file: string, options?: PatchOptions) => Effect.Effect<Patch>
  readonly patchCached: (cwd: string, file: string, options?: PatchOptions) => Effect.Effect<Patch>
  readonly patchAll: (cwd: string, ref: string, options?: PatchOptions) => Effect.Effect<Patch>
  readonly patchAllWorktree: (cwd: string, options?: PatchOptions) => Effect.Effect<Patch>
  readonly patchAllCached: (cwd: string, options?: PatchOptions) => Effect.Effect<Patch>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const diff = Effect.fn("Git.diff")(function* (cwd: string, ref: string, opts?: DiffOptions) {
      const list = nuls(
        yield* text(["diff", ...ws(opts), "--no-ext-diff", "--no-renames", "--name-status", "-z", ref, "--", "."], {
          cwd,
        }),
      )
    ...
    const diffWorktree = Effect.fn("Git.diffWorktree")(function* (cwd: string, opts?: DiffOptions) {
      const list = nuls(
        yield* text(["diff", ...ws(opts), "--no-ext-diff", "--no-renames", "--name-status", "-z", "--", "."], { cwd }),
      )
      return list.flatMap((code, idx) => {
        if (idx % 2 !== 0) return []
        const file = list[idx + 1]
    ...
    const diffCached = Effect.fn("Git.diffCached")(function* (cwd: string, opts?: DiffOptions) {
      const list = nuls(
        yield* text(
          ["diff", ...ws(opts), "--cached", "--no-ext-diff", "--no-renames", "--name-status", "-z", "--", "."],
          { cwd },
        ),
      )
      return list.flatMap((code, idx) => {
    ...
    const stats = Effect.fn("Git.stats")(function* (cwd: string, ref: string, opts?: DiffOptions) {
      return nuls(
        yield* text(["diff", ...ws(opts), "--no-ext-diff", "--no-renames", "--numstat", "-z", ref, "--", "."], {
          cwd,
        }),
      ).flatMap((item) => {
    ...
    const statsWorktree = Effect.fn("Git.statsWorktree")(function* (cwd: string, opts?: DiffOptions) {
      return nuls(
        yield* text(["diff", ...ws(opts), "--no-ext-diff", "--no-renames", "--numstat", "-z", "--", "."], { cwd }),
      ).flatMap(
        (item) => {
          const a = item.indexOf("\t")
          const b = item.indexOf("\t", a + 1)
    ...
    const statsCached = Effect.fn("Git.statsCached")(function* (cwd: string, opts?: DiffOptions) {
      return nuls(
        yield* text(
          ["diff", ...ws(opts), "--cached", "--no-ext-diff", "--no-renames", "--numstat", "-z", "--", "."],
          { cwd },
        ),
      ).flatMap((item) => {
        const a = item.indexOf("\t")
    ...
    const patch = Effect.fn("Git.patch")(function* (cwd: string, ref: string, file: string, options?: PatchOptions) {
      const result = yield* run(
        ["diff", ...ws(options), "--patch", "--no-ext-diff", "--no-renames", `--unified=${options?.context ?? 3}`, ref, "--", file],
    ...
    const patchWorktree = Effect.fn("Git.patchWorktree")(function* (cwd: string, file: string, options?: PatchOptions) {
      const result = yield* run(
        ["diff", ...ws(options), "--patch", "--no-ext-diff", "--no-renames", `--unified=${options?.context ?? 3}`, "--", file],
        { cwd, maxOutputBytes: options?.maxOutputBytes },
      )
      return { text: result.truncated ? "" : result.text(), truncated: result.truncated } satisfies Patch
    })

    const patchCached = Effect.fn("Git.patchCached")(function* (cwd: string, file: string, options?: PatchOptions) {
      const result = yield* run(
        [
          "diff",
          ...ws(options),
    ...
    const patchAll = Effect.fn("Git.patchAll")(function* (cwd: string, ref: string, options?: PatchOptions) {
      const result = yield* run(
        ["diff", ...ws(options), "--patch", "--no-ext-diff", "--no-renames", `--unified=${options?.context ?? 3}`, ref, "--", "."],
        { cwd, maxOutputBytes: options?.maxOutputBytes },
      )
      return { text: result.text(), truncated: result.truncated } satisfies Patch
    })

    const patchAllWorktree = Effect.fn("Git.patchAllWorktree")(function* (cwd: string, options?: PatchOptions) {
      const result = yield* run(
        ["diff", ...ws(options), "--patch", "--no-ext-diff", "--no-renames", `--unified=${options?.context ?? 3}`, "--", "."],
        { cwd, maxOutputBytes: options?.maxOutputBytes },
      )
      return { text: result.text(), truncated: result.truncated } satisfies Patch
    })

    const patchAllCached = Effect.fn("Git.patchAllCached")(function* (cwd: string, options?: PatchOptions) {
      const result = yield* run(
        ["diff", ...ws(options), "--cached", "--patch", "--no-ext-diff", "--no-renames", `--unified=${options?.context ?? 3}`, "--", "."],
        { cwd, maxOutputBytes: options?.maxOutputBytes },
      )
      return { text: result.text(), truncated: result.truncated } satisfies Patch
    })
