...
export const readOrQuarantine = (fs: AppFileSystem.Interface, target: string) =>
  fs.readJson(target).pipe(
    Effect.catchIf(
      (e): e is InstanceType<typeof AppFileSystem.FileSystemError> =>
        (e as any)?._tag === "FileSystemError" && (e as any)?.method === "readJson",
      () =>
        Effect.gen(function* () {
          yield* fs.rename(target, target + ".corrupt").pipe(Effect.ignore)
          log.error("quarantined corrupt json", { target })
          return yield* Effect.fail(new NotFoundError({ message: "Resource corrupt and quarantined" }))
        }),
    ),
  )
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const wrap = <A>(target: string, body: Effect.Effect<A, Error>) =>
    ...
    const read: Interface["read"] = <T>(key: string[]) =>
      Effect.gen(function* () {
        const value = yield* withResolved(key, (target, rw) =>
          TxReentrantLock.withReadLock(rw, wrap(target, readOrQuarantine(fs, target))),
        )
        return value as T
      })
      ...
      Effect.gen(function* () {
        const value = yield* withResolved(key, (target, rw) =>
          TxReentrantLock.withWriteLock(
            rw,
            Effect.gen(function* () {
              const content = yield* wrap(target, readOrQuarantine(fs, target))
              fn(content as T)
