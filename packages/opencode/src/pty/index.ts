...
import { createPtyOutputDecoder } from "./output-decoder"
...
/**
 * node-pty on Windows resolves bare names via PATH; Electron often ships a minimal PATH, so resolve explicitly.
 * Unlike `child_process.spawn({ shell: true })`, node-pty's `CreateProcess` path does not apply the same PATH
 * resolution helpers, so we resolve likely shells ourselves.
 */
function resolveWindowsPtyExecutable(command: string, env: Record<string, string>, cwd: string) {
  if (path.win32.isAbsolute(command)) return command
  if (command.includes("\\") || command.includes("/")) {
    const resolved = path.win32.resolve(cwd, command)
    if (existsSync(resolved)) return resolved
    return command
  }

  const fromPath = which(command, env)
  ...
  const fallbacks: Record<string, string> = {
    powershell: path.join(windir, "System32", "WindowsPowerShell", "v1.0", "powershell.exe"),
    "powershell.exe": path.join(windir, "System32", "WindowsPowerShell", "v1.0", "powershell.exe"),
    cmd: path.join(windir, "System32", "cmd.exe"),
    "cmd.exe": path.join(windir, "System32", "cmd.exe"),
  }
  const candidate = fallbacks[base]
...
export type WriteInput = Types.DeepMutable<Schema.Schema.Type<typeof WriteInput>>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Pty.create")(function* (input: CreateInput) {
      ...
      const spawnCommand =
        process.platform === "win32" ? resolveWindowsPtyExecutable(command, env, cwd) : command
      log.info("creating session", { id, cmd: command, resolved: spawnCommand, args, cwd })
      ...
      const output = createPtyOutputDecoder()
