  ...
  new Hono()
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Returns whether a workspace-relative or absolute path exists under the project directory.",
        operationId: "file.exists",
        responses: {
          200: {
            description: "Path existence",
            content: {
              "application/json": {
                schema: resolver(
                  z.object({
      ...
      async (c) =>
        jsonRequest("FileRoutes.exists", c, function* () {
          const svc = yield* File.Service
          return yield* svc.exists(c.req.valid("query").path)
        }),
    )
    .get(
      "/file/status",
