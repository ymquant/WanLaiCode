      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          sessions.length > limit && list.length > 0
            ? {
                "x-next-cursor": Session.formatGlobalListCursor({
                  archived: ctx.query.archived,
                  time: ctx.query.archived
                    ? (list[list.length - 1].time.archived ?? list[list.length - 1].time.updated)
                    : list[list.length - 1].time.updated,
