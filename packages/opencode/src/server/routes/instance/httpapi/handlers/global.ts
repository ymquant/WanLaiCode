...
import { afterGlobalConfigUpdate, disposeAllInstancesAndEmitGlobalDisposed } from "@/server/global-lifecycle"
...
export const globalHandlers = HttpApiBuilder.group(RootHttpApi, "global", (handlers) =>
  Effect.gen(function* () {
    ...
    const proxyGet = Effect.fn("GlobalHttpApi.proxyGet")(function* () {
      return yield* Effect.promise(() => NetProxy.visibleSystemProxy())
    })
    ...
    const configUpdate = Effect.fn("GlobalHttpApi.configUpdate")(function* (ctx) {
      ...
      afterGlobalConfigUpdate({
        ...
        config: ctx.payload,
        dispose: () => bridge.fork(disposeAllInstancesAndEmitGlobalDisposed({ swallowErrors: true })),
      })
      return result.info
    })
