...
import { ApiNotFoundError, ApiSteerEmptyInputError, ApiSteerTurnInactiveError } from "../errors"
...
export const MessagesQuery = Schema.Struct({
  ...
  // 桌面端历史首屏使用 compact；其它客户端省略时继续获得完整 patch，保持 API 向后兼容。
  summaryDiffs: Schema.optional(Schema.Literals(["full", "compact"])),
})
export const StatusMap = Schema.Record(Schema.String, SessionStatus.Info)
export const UpdatePayload = Schema.Struct({
  title: Schema.optional(Schema.String),
  permission: Schema.optional(Permission.Ruleset),
  // 两套 HTTP 路由共享同一模型更新结构，SDK 才能稳定驱动桌面与手机双向同步。
...
export const SetGoalPayload = Schema.Struct({
  objective: Schema.optional(Schema.String),
  // 用户经 HTTP 只能暂停/恢复：complete|blocked 是模型 update_goal 工具专属，usageLimited|budgetLimited 是系统专属，
  // 一律不允许从这个用户面端点写入（否则可把目标复活或置入非预期态，绕过 update_goal 的限制）
  status: Schema.optional(Schema.Literals(["active", "paused"])),
})
export const ForkPayload = Schema.Struct(Struct.omit(Session.ForkInput.fields, ["sessionID"]))
// 停止请求携带可选回合标识；不带标识时继续兼容旧版客户端。
export const AbortQuery = Schema.Struct({
  // Effect OpenAPI 必须与 MessageID.zod 的 startsWith 契约发布同一个 `^msg.*`，避免双后端 schema 漂移。
  turnID: Schema.optional(MessageID.pipe(Schema.check(Schema.isPattern(/^msg.*/)))),
})
...
// steer 直接复用核心契约，确保 targetTurnID 在两套 HTTP 路由中都是必填且不能伪造 sessionID。
export const SteerPayload = Schema.Struct(Struct.omit(SessionPrompt.SteerInput.fields, ["sessionID"]))
    ...
    HttpApiGroup.make("session")
      .add(
        ...
        HttpApiEndpoint.post("abort", SessionPaths.abort, {
          params: { sessionID: SessionID },
          // turnID 放在 query 中，避免旧版空 body 停止请求被 JSON 解码层拒绝。
          query: AbortQuery,
        ...
        HttpApiEndpoint.post("steer", SessionPaths.steer, {
          ...
          success: described(SessionPrompt.SteerAck, "Steer persisted"),
          error: [HttpApiError.BadRequest, HttpApiError.NotFound, ApiSteerEmptyInputError, ApiSteerTurnInactiveError],
        }).annotateMerge(
          OpenApi.annotations({
        ...
        HttpApiEndpoint.post("setGoal", SessionPaths.goal, {
          params: { sessionID: SessionID },
          payload: SetGoalPayload,
          success: described(Goal, "Updated goal"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "session.setGoal",
            summary: "Set session goal",
            description: "Create or update the autonomous goal for a session.",
          }),
        ),
        HttpApiEndpoint.get("getGoal", SessionPaths.goal, {
          params: { sessionID: SessionID },
          success: described(Schema.NullOr(Goal), "Session goal"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "session.getGoal",
            summary: "Get session goal",
            description: "Retrieve the autonomous goal associated with a session, if any.",
          }),
        ),
        HttpApiEndpoint.delete("clearGoal", SessionPaths.goal, {
          params: { sessionID: SessionID },
          success: described(Schema.Boolean, "Cleared goal"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "session.clearGoal",
            summary: "Clear session goal",
            description: "Remove the autonomous goal from a session.",
          }),
        ),
      )
      .annotateMerge(
