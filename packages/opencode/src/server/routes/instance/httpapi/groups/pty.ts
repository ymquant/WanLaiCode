  ...
  .add(
    HttpApiGroup.make("pty")
      .add(
        ...
        HttpApiEndpoint.post("write", PtyPaths.write, {
          params: { ptyID: PtyID },
          payload: Pty.WriteInput,
          success: described(Schema.Boolean, "Data written"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "pty.write",
            summary: "Write data to PTY session",
            description: "Write data to an existing pseudo-terminal (PTY) session.",
          }),
        ),
      )
