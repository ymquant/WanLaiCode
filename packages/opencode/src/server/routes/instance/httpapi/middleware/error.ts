...
import { isVcsOperationFailedError } from "@/project/vcs"
...
export function internalErrorBody() {
  return { name: "InternalError", data: { reason: "INTERNAL_ERROR", message: "Internal server error" } }
}
...
export const errorLayer = HttpRouter.middleware<{ handles: unknown }>()((effect) =>
  effect.pipe(
    Effect.catchCause((cause) => {
      ...
      return Effect.succeed(HttpServerResponse.jsonUnsafe(internalErrorBody(), { status: 500 }))
    }),
  ),
).layer
