...
const QueryBooleanParameters = new Set(["roots", "archived", "refresh"])
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  for (const [path, item] of Object.entries(spec.paths ?? {})) {
    ...
    for (const method of ["get", "post", "put", "delete", "patch"] as const) {
      ...
      if (path === "/session/{sessionID}/goal" && method === "get") {
        const content200 = operation.responses?.["200"]?.content?.["application/json"]
        if (content200?.schema) content200.schema = nullable(content200.schema)
      }
  ...
  // 联合错误响应不会被单一响应兼容逻辑整体替换，删除 Effect 组件后仍需把其中的分支引用指向 legacy schema。
  rewriteRefs(spec, "EffectHttpApiErrorBadRequest", "BadRequestError")
  rewriteRefs(spec, "effect_HttpApiError_BadRequest", "BadRequestError")
  rewriteRefs(spec, "EffectHttpApiErrorNotFound", "NotFoundError")
  rewriteRefs(spec, "effect_HttpApiError_NotFound", "NotFoundError")
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  if (schemas.Goal?.properties?.tokenBudget)
    schemas.Goal.properties.tokenBudget = nullable(schemas.Goal.properties.tokenBudget)
