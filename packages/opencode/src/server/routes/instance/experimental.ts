    ...
    .get(
      ...
      validator(
        ...
        z.object({
          ...
          cursor: z.string().optional().meta({ description: "Pagination cursor (timestamp or timestamp:id for archived lists)" }),
      ...
      async (c) => {
        ...
        if (hasMore && list.length > 0) {
          ...
          c.header(
            "x-next-cursor",
            Session.formatGlobalListCursor({
              archived: queryBoolean(query.archived),
              time: queryBoolean(query.archived) ? (last.time.archived ?? last.time.updated) : last.time.updated,
              id: last.id,
            }),
          )
        }
