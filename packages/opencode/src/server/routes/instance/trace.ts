...
import { publicErrorResponse } from "@/server/public-error-response"
...
export async function jsonRequest<C extends Context, A, E>(
  ...
  const exit = await AppRuntime.runPromiseExit(
    Effect.gen(() => effect(c)).pipe(Effect.withSpan(name, { attributes: requestAttributes(c) })),
  )
  if (Exit.isSuccess(exit)) return c.json(exit.value)
  const response = publicErrorResponse(Cause.squash(exit.cause))
