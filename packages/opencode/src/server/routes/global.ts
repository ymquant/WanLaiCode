...
import { afterGlobalConfigUpdate, disposeAllInstancesAndEmitGlobalDisposed } from "../global-lifecycle"
...
export const GlobalRoutes = lazy(() =>
  new Hono()
    ...
    .get(
      "/proxy",
      describeRoute({
        summary: "Get detected system proxy",
        description: "Read the proxy environment currently visible to app-owned HTTP and HTTPS requests.",
        operationId: "global.proxy.get",
        responses: {
          200: {
            description: "Detected system proxy information",
            content: {
              "application/json": {
                schema: resolver(
                  z.object({
      ...
      async (c) => c.json(await NetProxy.visibleSystemProxy()),
    )
    .get(
      "/config",
      describeRoute({
      ...
      async (c) => {
        ...
        const result = await AppRuntime.runPromise(
          Config.Service.use((cfg) =>
            Effect.gen(function* () {
              const result = yield* cfg.updateGlobal(config)
              return result
            }),
          ),
        )
        afterGlobalConfigUpdate({
