...
import { WanlaiCodeUserCenterEvents } from "./wanlaicode-user-center-events"
import { WanlaiCodeRefreshCoordinator } from "@/provider/wanlaicode-refresh-coordinator"
import { RemoteControlGateway, remoteControlAvailable } from "@/remote-control/gateway"
...
function createHono(opts: CorsOptions, selection: ServerBackend.Selection = ServerBackend.force(select(), "hono")) {
  ...
  const app = new Hono()
    ...
    // 该路由只监听现有 sidecar 地址，绝不额外暴露端口或修改 host。
    .route("/global/remote-control", RemoteControlRoutes())

  const runtime = adapter.create(app)

  if (Flag.WANLAICODE_WORKSPACE_ID) {
    return {
      app: app
        .use(
          InstanceMiddleware(Flag.WANLAICODE_WORKSPACE_ID ? WorkspaceID.make(Flag.WANLAICODE_WORKSPACE_ID) : undefined),
        )
        .use(FenceMiddleware)
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  const { ensureWindowsPath } = await import("@/util/which")
  ensureWindowsPath()
  const selected = select()
  ...
  // 仅在真正 serve 时启动自动化调度循环(避免 generate 等构建期进程被定时器挂住)
  AutomationScheduler.ensureScheduler()
  GoalRuntime.ensureGoalRuntime()
  const stopWanlaiCodeUserCenterEvents = WanlaiCodeUserCenterEvents.start()
  WanlaiCodeRefreshCoordinator.ensureTokenRefreshScheduler()
  // 只有 desktop sidecar 能持有远控；普通 CLI 与工作区即使共享 auth.json，也不能建立第二条消费连接。
  if (remoteControlAvailable(Flag.WANLAICODE_WORKSPACE_ID, Flag.WANLAICODE_CLIENT)) RemoteControlGateway.start()
  ...
  return {
    ...
    stop(close?: boolean) {
      unpublish()
      stopWanlaiCodeUserCenterEvents()
      WanlaiCodeRefreshCoordinator.stopTokenRefreshScheduler()
      RemoteControlGateway.stop()
