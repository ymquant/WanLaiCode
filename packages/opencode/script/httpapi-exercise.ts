...
const preserveExerciseGlobalRoot = !!(process.env.WANLAICODE_HTTPAPI_EXERCISE_GLOBAL ?? process.env.OPENCODE_HTTPAPI_EXERCISE_GLOBAL)
const exerciseGlobalRoot =
  (process.env.WANLAICODE_HTTPAPI_EXERCISE_GLOBAL ?? process.env.OPENCODE_HTTPAPI_EXERCISE_GLOBAL) ??
  path.join(process.env.TMPDIR ?? "/tmp", `opencode-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.WANLAICODE_DISABLE_SHARE = "true"
...
const preserveExerciseDatabase = !!(process.env.WANLAICODE_HTTPAPI_EXERCISE_DB ?? process.env.OPENCODE_HTTPAPI_EXERCISE_DB)
const exerciseDatabasePath =
  (process.env.WANLAICODE_HTTPAPI_EXERCISE_DB ?? process.env.OPENCODE_HTTPAPI_EXERCISE_DB) ??
  path.join(process.env.TMPDIR ?? "/tmp", `opencode-httpapi-exercise-${process.pid}.db`)
process.env.WANLAICODE_DB = exerciseDatabasePath
Flag.WANLAICODE_DB = exerciseDatabasePath
...
type ScenarioContext = {
  ...
  session: (input?: { title?: string; parentID?: SessionID; id?: SessionID }) => Effect.Effect<SessionInfo>
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "wanlaicode.jsonc"),
  ...
  http
    ...
    .at((ctx) => ({ path: "/vcs/diff?mode=unstaged", headers: ctx.headers() }))
    .json(200, array),
          ...
          session: (input) =>
            run(
              modules.Session.Service.use((svc) =>
                svc.create({ id: input?.id, title: input?.title, parentID: input?.parentID }),
              ),
            ).pipe(Effect.orDie),
...
function app(modules: Runtime, backend: Backend) {
  Flag.WANLAICODE_EXPERIMENTAL_HTTPAPI = backend === "effect"
  Flag.WANLAICODE_SERVER_PASSWORD = undefined
  Flag.WANLAICODE_SERVER_USERNAME = undefined
