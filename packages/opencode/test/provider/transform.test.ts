    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".wanlaicode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const wanlaicodeModel = {
      ...openaiModel,
      providerID: "wanlaicode",
      api: {
        id: "wanlaicode-test",
        url: "https://api.wanlai.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, wanlaicodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.wanlaicode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.wanlaicode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          wanlaicode: { itemId: "msg_wanlaicode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              wanlaicode: { itemId: "msg_wanlaicode_part" },
    ...
    expect(result[0].providerOptions?.wanlaicode?.itemId).toBe("msg_wanlaicode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.wanlaicode?.itemId).toBe("msg_wanlaicode_part")
...
describe("ProviderTransform.variants", () => {
  ...
  describe("@ai-sdk/openai-compatible", () => {
    ...
    test("openai-compatible: wanlaicode 优先用下发的 reasoning_efforts 生成 variants", () => {
      const model = {
        id: "gpt-5.5",
        providerID: "wanlaicode",
        api: { id: "gpt-5.5", npm: "@ai-sdk/openai-compatible", url: "" },
        capabilities: { reasoning: true },
        release_date: "2024-01-01",
        reasoning_efforts: ["none", "low", "medium", "high", "xhigh"],
      } as any
      const v = ProviderTransform.variants(model)
      expect(Object.keys(v)).toContain("xhigh")
      expect(v["xhigh"]).toEqual({ reasoningEffort: "xhigh" })
    })

    test("openai-compatible: wanlaicode 优先用 reasoning_options 覆盖旧 reasoning_efforts", () => {
      const model = {
        id: "deepseek-v4-pro",
        providerID: "wanlaicode",
        api: { id: "deepseek-v4-pro", npm: "@ai-sdk/openai-compatible", url: "" },
        capabilities: { reasoning: true },
        release_date: "2024-01-01",
      ...
      const v = ProviderTransform.variants(model)
      expect(Object.keys(v)).toEqual(["high"])
      expect(v.high).toEqual({ reasoningEffort: "high" })
    })

    test("openai-compatible: 非 wanlaicode provider 即使带 reasoning_efforts 也走旧启发式(不受下发影响)", () => {
      const model = {
        id: "some-model",
        providerID: "cerebras",
        api: { id: "some-model", npm: "@ai-sdk/openai-compatible", url: "" },
        capabilities: { reasoning: true },
        release_date: "2024-01-01",
        reasoning_efforts: ["none", "low", "medium", "high", "xhigh"],
      } as any
    ...
    test("openai-compatible: 无 reasoning_efforts 时回退旧启发式", () => {
      const model = {
        id: "some-oss-model",
        api: { id: "some-oss-model", npm: "@ai-sdk/openai-compatible", url: "" },
        capabilities: { reasoning: true },
        release_date: "2024-01-01",
      } as any
      const v = ProviderTransform.variants(model)
