...
describe("instance HttpApi", () => {
  ...
  test("vcs push failure returns message body through Hono bridge", async () => {
    await using tmp = await tmpdir({ git: true })

    const response = await app().request(InstancePaths.vcsPush, {
      method: "POST",
      headers: { "x-opencode-directory": tmp.path, "content-type": "application/json" },
      body: JSON.stringify({}),
    })
    ...
    expect(body.name).toBe("VcsPushFailedError")
