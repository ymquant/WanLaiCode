...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = (process.env.WANLAICODE_EXPERIMENTAL_DISABLE_FILEWATCHER ?? process.env.OPENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER)
  process.env.WANLAICODE_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) {
    delete process.env.WANLAICODE_EXPERIMENTAL_DISABLE_FILEWATCHER
    delete process.env.OPENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER
  }
    else process.env.WANLAICODE_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
...
async function fillSummaryMessage(sessionID: SessionID, count: number, patchPrefix: string) {
  const id = MessageID.ascending()
  await svc.updateMessage({
    id,
    sessionID,
...
describe("session messages endpoint", () => {
  ...
  test("compacts only oversized summaries when requested", async () => {
    await using tmp = await tmpdir({ git: true })
    await withoutWatcher(() =>
      WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          const session = await svc.create({})
          const large = await fillSummaryMessage(session.id, 121, "large")
          const boundary = await fillSummaryMessage(session.id, 120, "boundary")
          const app = Server.Default().app

          const full = await app.request(`/session/${session.id}/message`)
          const fullBody = (await full.json()) as MessageV2.WithParts[]
          const fullLarge = fullBody.find((item) => item.info.id === large)
          const fullBoundary = fullBody.find((item) => item.info.id === boundary)
          expect(fullLarge?.info.role === "user" && fullLarge.info.summary?.diffs[0]?.patch).toBe("large-0")
          expect(fullBoundary?.info.role === "user" && fullBoundary.info.summary?.diffs[0]?.patch).toBe("boundary-0")

          const compact = await app.request(`/session/${session.id}/message?summaryDiffs=compact`)
          const compactBody = (await compact.json()) as MessageV2.WithParts[]
          const compactLarge = compactBody.find((item) => item.info.id === large)
          const compactBoundary = compactBody.find((item) => item.info.id === boundary)
          // compact 只裁掉超过阈值的 patch 正文，文件元数据和边界行为保持不变。
