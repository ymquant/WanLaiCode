...
const original = {
  ...
  envPassword: (process.env.WANLAICODE_SERVER_PASSWORD ?? process.env.OPENCODE_SERVER_PASSWORD),
  envUsername: (process.env.WANLAICODE_SERVER_USERNAME ?? process.env.OPENCODE_SERVER_USERNAME),
}
const auth = { username: "wanlaicode", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) {
    delete process.env.WANLAICODE_SERVER_PASSWORD
    delete process.env.OPENCODE_SERVER_PASSWORD
  }
  else process.env.WANLAICODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) {
    delete process.env.WANLAICODE_SERVER_USERNAME
    delete process.env.OPENCODE_SERVER_USERNAME
  }
  else process.env.WANLAICODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener(backend: "effect-httpapi" | "hono" = "effect-httpapi") {
  Flag.WANLAICODE_EXPERIMENTAL_HTTPAPI = backend === "effect-httpapi"
  Flag.WANLAICODE_SERVER_PASSWORD = auth.password
  Flag.WANLAICODE_SERVER_USERNAME = auth.username
  process.env.WANLAICODE_SERVER_PASSWORD = auth.password
  process.env.WANLAICODE_SERVER_USERNAME = auth.username
