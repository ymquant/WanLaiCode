...
describe("Runner", () => {
  ...
  // --- awaitIdle (drain) semantics ---

  it.live(
    "awaitIdle waits for running work to finish without interrupting it",
    Effect.gen(function* () {
      const s = yield* Scope.Scope
      const runner = Runner.make<string>(s)
      const gate = yield* Deferred.make<void>()
      const fiber = yield* runner.ensureRunning(Deferred.await(gate).pipe(Effect.as("done"))).pipe(Effect.forkChild)
      yield* Effect.sleep("10 millis")
      expect(runner.busy).toBe(true)

      // awaitIdle 不能提前返回：回合还没跑完就 resolve 会导致后续销毁把它砍断
      const idle = yield* runner.awaitIdle.pipe(Effect.forkChild)
      const early = yield* Fiber.await(idle).pipe(Effect.timeout("20 millis"), Effect.exit)
      expect(Exit.isFailure(early)).toBe(true)

      yield* Deferred.succeed(gate, undefined)
      yield* Fiber.await(idle).pipe(Effect.timeout("250 millis"))

      const runExit = yield* Fiber.await(fiber)
      expect(Exit.isSuccess(runExit)).toBe(true)
      if (Exit.isSuccess(runExit)) expect(runExit.value).toBe("done")
      expect(runner.busy).toBe(false)
    }),
  )

  it.live(
    "awaitIdle on an idle runner resolves immediately",
    Effect.gen(function* () {
      const s = yield* Scope.Scope
      const runner = Runner.make<string>(s)
      yield* runner.awaitIdle.pipe(Effect.timeout("100 millis"))
      expect(runner.busy).toBe(false)
    }),
  )
