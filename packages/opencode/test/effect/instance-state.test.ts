...
it.live("InstanceState invalidates every loaded directory", () =>
  Effect.gen(function* () {
    const one = yield* tmpdirScoped()
    const two = yield* tmpdirScoped()
    let n = 0
...
it.live("InstanceState stops tracking disposed directories", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const state = yield* InstanceState.make(() => Effect.sync(() => ({})))
    yield* access(state, dir)
    expect(state.directories.has(dir)).toBe(true)

    yield* Effect.promise(() => disposeTestInstance({ directory: dir }))
    expect(state.directories.has(dir)).toBe(false)
  }),
)

it.live("InstanceState removes tracking before disposal completes", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    ...
    const disposal = yield* Effect.promise(() => disposeTestInstance({ directory: dir })).pipe(Effect.forkScoped)
    ...
    expect(state.directories.has(dir)).toBe(true)
    yield* InstanceState.invalidateAll(state)
    expect((yield* access(state, dir)).n).toBe(3)
  }),
)
...
it.live("dispose waits for registered drainers before tearing down the instance", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    // access 会 store.load，建立实例缓存
    const state = yield* InstanceState.make(() => Effect.sync(() => ({})))
    yield* access(state, dir)
    ...
    const off = registerDrainer((directory) => {
      if (directory !== dir) return Effect.void
      drainCalled = true
      return Effect.promise(() => drained)
    })

    let disposeDone = false
    const disposePromise = disposeTestInstance({ directory: dir }).then(() => {
      disposeDone = true
    })

    // drainer 未完成前，dispose 必须一直阻塞，不能拆实例（避免砍断在跑的回合）
    yield* Effect.sleep("80 millis")
    expect(drainCalled).toBe(true)
    expect(disposeDone).toBe(false)

    releaseDrain()
    yield* Effect.promise(() => disposePromise)
    expect(disposeDone).toBe(true)
    off()
  }),
)

it.live("reload waits for registered drainers before tearing down the previous instance", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const state = yield* InstanceState.make(() => Effect.sync(() => ({})))
    yield* access(state, dir)
    ...
    const reloadPromise = reloadTestInstance({ directory: dir }).then(() => {
      reloadDone = true
    })

    // project-reload 也必须先排空活跃回合再拆旧实例，否则会砍断正在生成的回答
    yield* Effect.sleep("80 millis")
    expect(drainCalled).toBe(true)
    expect(reloadDone).toBe(false)

    releaseDrain()
    yield* Effect.promise(() => reloadPromise)
    expect(reloadDone).toBe(true)
    off()
  }),
)

it.live("reload keeps the old instance loadable during drain (stop-request escape hatch)", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    let n = 0
    const state = yield* InstanceState.make(() => Effect.sync(() => ({ n: ++n })))
    const first = yield* access(state, dir)

    let releaseDrain: () => void = () => {}
    const drained = new Promise<void>((resolve) => (releaseDrain = resolve))
    const off = registerDrainer((directory) => (directory !== dir ? Effect.void : Effect.promise(() => drained)))

    const reloadPromise = reloadTestInstance({ directory: dir })
    yield* Effect.sleep("80 millis")

    // 排空旧回合期间，load 绝不能被卡在新实例的未决 deferred 上——必须仍返回旧实例，
    // 否则 session abort / 停止请求到不了旧 runner，用户无法取消。
    const during = yield* access(state, dir).pipe(Effect.timeout("1 second"))
