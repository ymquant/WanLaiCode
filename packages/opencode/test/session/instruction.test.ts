...
describe("Instruction.resolve", () => {
  ...
  it.live("discovers both AGENTS.md and CLAUDE.md in subdirectory", () =>
    withFiles(
      { "subdir/AGENTS.md": "# Subdir Agents", "subdir/CLAUDE.md": "# Subdir Claude", "subdir/nested/file.ts": "const x = 1" },
      (dir) =>
        Effect.gen(function* () {
          const svc = yield* Instruction.Service
          const results = yield* svc.resolve([], path.join(dir, "subdir", "nested", "file.ts"), MessageID.make("message-test-both"))
          expect(results.length).toBe(2)
          expect(results.some((r) => r.filepath === path.join(dir, "subdir", "AGENTS.md"))).toBe(true)
          expect(results.some((r) => r.filepath === path.join(dir, "subdir", "CLAUDE.md"))).toBe(true)
        }),
    ),
  )
...
describe("Instruction.system", () => {
  it.live("imports automatic instruction files only from the project root", () =>
    Effect.gen(function* () {
      const root = yield* tmpdirScoped({ git: true })
      yield* writeFiles(root, {
        "AGENTS.md": "# Root Agents",
        "CLAUDE.md": "# Root Claude",
        "packages/app/AGENTS.md": "# Nested Agents",
        "packages/app/CLAUDE.md": "# Nested Claude",
      })

      yield* Effect.gen(function* () {
        const svc = yield* Instruction.Service
        const paths = yield* svc.systemPaths()
        expect(paths.has(path.join(root, "AGENTS.md"))).toBe(true)
        expect(paths.has(path.join(root, "CLAUDE.md"))).toBe(true)
        expect(paths.has(path.join(root, "packages", "app", "AGENTS.md"))).toBe(false)
        expect(paths.has(path.join(root, "packages", "app", "CLAUDE.md"))).toBe(false)
      }).pipe(
        provideInstance(path.join(root, "packages", "app")),
        provideInstruction({ home: path.join(root, "home"), config: path.join(root, "config") }),
      )
    }),
  )
  ...
  it.live("loads enabled root AGENTS.md and root CLAUDE.md", () =>
    withFiles(
      {
        "AGENTS.md": "# Agents",
        "CLAUDE.md": "# Claude",
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          expect(paths.has(path.join(dir, "CLAUDE.md"))).toBe(true)

          const rules = yield* svc.system()
          expect(rules.some((rule) => rule.includes("AGENTS.md") && rule.includes("# Agents"))).toBe(true)
          expect(rules.some((rule) => rule.includes("CLAUDE.md") && rule.includes("# Claude"))).toBe(true)
        }),
    ),
  )

  it.live("respects instruction import settings and injects custom rules", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        yield* writeFiles(dir, { "AGENTS.md": "# Agents", "CLAUDE.md": "# Claude" })
        const svc = yield* Instruction.Service
        const paths = yield* svc.systemPaths()
        expect(paths.has(path.join(dir, "AGENTS.md"))).toBe(false)
        expect(paths.has(path.join(dir, "CLAUDE.md"))).toBe(true)
        expect(yield* svc.system()).toContain("Rule: Testing\nAlways add focused tests.")
      }).pipe(
        Effect.provide(
          Instruction.layer.pipe(
            Layer.provide(
              TestConfig.layer({
                get: () =>
                  Effect.succeed({
                    instruction_import: { agents_md: false, claude_md: true },
                    rules: [{ id: "testing", title: "Testing", content: "Always add focused tests.", enabled: true }],
                  }),
              }),
            ),
            Layer.provide(AppFileSystem.defaultLayer),
            Layer.provide(FetchHttpClient.layer),
            Layer.provide(Global.layerWith({ home: path.join(dir, "home"), config: path.join(dir, "config") })),
          ),
        ),
      ),
    ),
