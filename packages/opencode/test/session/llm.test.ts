...
import { LLM, systemMessages } from "../../src/session/llm"
...
describe("session.llm.systemMessages", () => {
  test("keeps project instructions separate from the provider prompt", () => {
    expect(systemMessages("provider", ["Instructions from: C:\\project\\CLAUDE.md\nrule"])).toEqual([
      "provider",
      "Instructions from: C:\\project\\CLAUDE.md\nrule",
    ])
  })

  test("returns only instructions when header is empty", () => {
    expect(systemMessages("", ["rule-a", "rule-b"])).toEqual(["rule-a", "rule-b"])
  })

  test("filters falsy entries", () => {
    expect(systemMessages("header", ["a", "", "b"])).toEqual(["header", "a", "b"])
  })

  test("produces output that rejoins to 2-part structure for caching", () => {
    const system = systemMessages("provider", ["AGENTS.md content", "CLAUDE.md content", "user instruction"])
    expect(system.length).toBe(4)
    const header = system[0]
    // Simulate the rejoining logic in llm.ts stream()
    if (system.length > 2 && system[0] === header) {
      const rest = system.slice(1)
      system.length = 0
      system.push(header, rest.join("\n"))
    }
    expect(system).toEqual(["provider", "AGENTS.md content\nCLAUDE.md content\nuser instruction"])
...
describe("session.llm.stream", () => {
  ...
  test("classifies a stalled SSE stream as a retryable error", async () => {
    const server = state.server
    ...
    const fixture = await loadFixture(providerID, modelID)
    const model = fixture.model
    // 建立 SSE 后只发一个 chunk 就挂住，触发停滞看门狗（chunkTimeout）
    waitStreamingRequest("/chat/completions")
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const resolved = await getModel(ProviderID.make(providerID), ModelID.make(model.id))
        const sessionID = SessionID.make("session-test-stall")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const exit = await llm.runPromiseExit((svc) =>
          svc
            ...
            .pipe(
              Stream.tap((e) => Effect.sync(() => events.push(e as { type: string; error?: unknown }))),
              Stream.runDrain,
            ),
        )

        const errorEvent = events.find((e) => e.type === "error")
        const raw = errorEvent ? errorEvent.error : Exit.isFailure(exit) ? Cause.squash(exit.cause) : undefined
        expect(raw).toBeDefined()

        // 锁定走的是 STREAM_STALL 专用分支(而非碰巧也可重试的通用 APICallError):
        // 证明停滞错误的 code 原样穿过 AI SDK 到达 fromError、未被包装。
        expect((raw as { code?: string }).code).toBe("STREAM_STALL")

        const classified = MessageV2.fromError(raw, { providerID: ProviderID.make(providerID) })
        expect(MessageV2.APIError.isInstance(classified)).toBe(true)
        expect((classified as MessageV2.APIError).data.isRetryable).toBe(true)
        expect((classified as MessageV2.APIError).data.metadata?.code).toBe("STREAM_STALL")
      },
    })
  })
  ...
  test("uses OpenAI native image_generation tool for ordinary GPT-5 models", async () => {
    const server = state.server
    if (!server) {
      throw new Error("Server not initialized")
    }
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const resolved = await getModel(ProviderID.openai, ModelID.make(model.id))
        const sessionID = SessionID.make("session-test-native-image-tool")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const user = {
          id: MessageID.make("user-native-image-tool"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
        ...
        await drain({
          ...
          tools: {
            image_generation: tool({
              ...
              execute: async () => ({ output: "Generated 1 image." }),
            }),
          },
        ...
        expect(capture.url.pathname.endsWith("/responses")).toBe(true)
        expect(tools).toContainEqual(expect.objectContaining({ type: "image_generation", output_format: "png" }))
        expect(tools.some((item) => item.type === "function" && item.name === "image_generation")).toBe(false)
        expect(capture.body.tool_choice).toEqual({ type: "image_generation" })
      },
    })
  })
