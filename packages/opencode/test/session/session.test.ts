...
function waitFor(predicate: () => boolean, ms = 2_000, label = "condition") {
  const started = Date.now()
  return new Promise<void>((resolve, reject) => {
    const check = () => {
      if (predicate()) return resolve()
      if (Date.now() - started >= ms) return reject(new Error(`waitFor(${label}) timed out after ${ms}ms`))
      setTimeout(check, 25)
    }
    check()
  })
  ...
  test("should emit session.created event when session is created", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        await waitFor(() => eventReceived, 2_000, "session.created received")
        unsub()
  ...
  test("session.created event should be emitted before session.updated", async () => {
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const info = await create({})
        // Ordering assertion only — this is a precondition wait for both events to
        // arrive, not a latency check. Slow CI runners (esp. Windows) can take >2s,
        // so give generous headroom to avoid false-timeout flakes.
        await waitFor(
          () => events.includes("created") && events.includes("updated"),
          15_000,
          "session.created and session.updated ordering precondition",
        )
        unsubCreated()
        unsubUpdated()
...
describe("session.create idempotency", () => {
  test("returns an existing session when create is called with the same id", async () => {
    await WithInstance.provide({
      directory: projectRoot,
      fn: async () => {
  ...
  test("rejects create when the supplied id belongs to another project", async () => {
    await using tmp = await tmpdir()
    const foreign = await WithInstance.provide({
      directory: projectRoot,
      fn: () => create({}),
    })
    try {
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          await expect(create({ id: foreign.id })).rejects.toMatchObject({ name: "NotFoundError" })
        },
      })
    } finally {
      await WithInstance.provide({
        directory: projectRoot,
        fn: () => remove(foreign.id),
      })
    }
  })
})
