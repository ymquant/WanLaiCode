...
// 自定义消息 ID 用于覆盖远控/导入历史；真实先后必须由持久化时间线决定，不能依赖 ID 字典序。
async function customUser(sessionID: SessionID, id: MessageID, text: string, created: number) {
  const msg = await svc.updateMessage({
    id,
    role: "user",
    sessionID,
...
function fake(
  ...
  return {
    ...
    completeToolCall: Effect.fn("TestSessionProcessor.completeToolCall")(() => Effect.succeed(undefined)),
    failToolCall: Effect.fn("TestSessionProcessor.failToolCall")(() => Effect.succeed(false)),
    fail: Effect.fn("TestSessionProcessor.fail")(() => Effect.void),
...
describe("session.compaction.shouldCompact", () => {
  it.live(
    "triggers compaction from current message size when upstream usage is zero",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 100_000, output: 32_000 }) // usable = context - output = 68_000
        const sessionID = SessionID.make("ses_shouldcompact")
        // 上一轮自报 token 全 0（模拟网关未把 usage 透传给客户端），且与本轮同模型
        const lastFinished: MessageV2.Assistant = {
          id: MessageID.ascending(),
          role: "assistant",
          sessionID,
          mode: "build",
        ...
        // 当前要发送的消息体积巨大，按实际内容预估远超 usable —— usage 失真也必须能判定溢出
        ...
        const messages: MessageV2.WithParts[] = [
          {
            ...
            parts: [{ id: PartID.ascending(), messageID, sessionID, type: "text", text: "x".repeat(1_000_000) }],
          },
        ]
        expect(yield* compact.shouldCompact({ lastFinished, messages, model })).toBe(true)
      }),
    ),
  )

  it.live(
    "does not compact when neither message size nor usage overflows",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 200_000, output: 32_000 }) // usable = 168_000
        const sessionID = SessionID.make("ses_shouldcompact_ok")
        const lastFinished: MessageV2.Assistant = {
          id: MessageID.ascending(),
          role: "assistant",
          sessionID,
        ...
        const messages: MessageV2.WithParts[] = [
          {
            ...
            parts: [{ id: PartID.ascending(), messageID, sessionID, type: "text", text: "hello" }],
          },
        ]
        expect(yield* compact.shouldCompact({ lastFinished, messages, model })).toBe(false)
      }),
    ),
  )

  it.live(
    "compacts via same-model usage even when the current message is small",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 100_000, output: 32_000 }) // usable = 68_000
        const sessionID = SessionID.make("ses_shouldcompact_usage")
        // 同模型且上一轮自报 usage 已超 usable（70_000 >= 68_000）→ 应靠 usage 路径提前触发
        const lastFinished: MessageV2.Assistant = {
          id: MessageID.ascending(),
          role: "assistant",
          sessionID,
          ...
          path: { cwd: "/tmp", root: "/tmp" },
          cost: 0,
          tokens: { input: 70_000, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
          modelID: ModelID.make("test-model"),
          providerID: ProviderID.make("test"),
          parentID: MessageID.ascending(),
          time: { created: Date.now() },
        ...
        const messages: MessageV2.WithParts[] = [
          {
            ...
            parts: [{ id: PartID.ascending(), messageID, sessionID, type: "text", text: "hi" }],
          },
        ]
        expect(yield* compact.shouldCompact({ lastFinished, messages, model })).toBe(true)
      }),
  ...
  it.live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        // 压缩 marker 必须持久化触发它的逻辑回合，供重启后的 runner 恢复。
        expect(msgs[0].info.turnID).toBe(turnID)
...
describe("session.compaction.prune", () => {
  ...
  it.live(
    "keeps tool output protected when a queued turn splits one physical steering turn",
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
          ...
          const info = yield* ssn.create({})
          const turnID = MessageID.ascending()
          const root = yield* ssn.updateMessage({
            id: turnID,
            role: "user",
            turnID,
            sessionID: info.id,
          ...
          const firstReply: MessageV2.Assistant = {
            id: MessageID.ascending(),
            role: "assistant",
            turnID,
            sessionID: info.id,
          ...
          yield* ssn.updateMessage(firstReply)
          yield* ssn.updatePart({
            id: PartID.ascending(),
          ...
          const queuedTurnID = MessageID.ascending()
          const queued = yield* ssn.updateMessage({
            id: queuedTurnID,
            role: "user",
            turnID: queuedTurnID,
            sessionID: info.id,
          ...
          const firstSteer = yield* ssn.updateMessage({
            id: MessageID.ascending(),
            role: "user",
            turnID,
            steerTargetTurnID: turnID,
            sessionID: info.id,
            agent: "build",
            model: ref,
            time: { created: Date.now() },
          ...
          expect(part?.type).toBe("tool")
          // A 的早期工具、队列 B、steer A 虽然交错，A 与 B 仍是最近两个受保护物理回合。
          if (part?.type === "tool" && part.state.status === "completed") {
            expect(part.state.time.compacted).toBeUndefined()
          }
        }),
...
describe("session.compaction.process", () => {
  ...
  test("counts a steering turn once when a queued turn is interleaved", async () => {
    await using tmp = await tmpdir()
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const session = await svc.create({})
        await user(session.id, "history")
        const guided = await user(session.id, "guided root")
        await assistant(session.id, guided.id, tmp.path)
        const queued = await user(session.id, "queued turn")
        const steer = await svc.updateMessage({
          id: MessageID.ascending(),
          role: "user",
          turnID: guided.id,
          steerTargetTurnID: guided.id,
          sessionID: session.id,
          agent: "build",
          model: ref,
          time: { created: Date.now() },
        ...
        await svc.updatePart({
          ...
          sessionID: session.id,
          type: "text",
          text: "guided follow-up",
        })
        await assistant(session.id, steer.id, tmp.path)
        await assistant(session.id, queued.id, tmp.path)
        await SessionCompaction.create({
          sessionID: session.id,
          agent: "build",
          model: ref,
        ...
        try {
          ...
          const part = await lastCompactionPart(session.id)
          // 最近两个物理回合是 guided(A) 与 queued(B)；中间的 B 不能让后到 steer A 把 A 重复计数。
          expect(part?.tail_start_id).toBe(guided.id)
        } finally {
          await rt.dispose()
        }
      },
  ...
  test("keeps the earliest existing tail anchor by persisted timeline order", async () => {
    await using tmp = await tmpdir()
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const session = await svc.create({})
        const now = Date.now()
        await customUser(session.id, MessageID.make("msg_000_history"), "history", now - 2)
        const preserved = await customUser(session.id, MessageID.make("msg_zzz_deferred"), "deferred queue", now - 1)
        const selected = await customUser(
          session.id,
          MessageID.make("msg_aaa_natural_tail"),
          "natural recent tail",
          now,
        )
        await SessionCompaction.create({
          ...
          model: ref,
          auto: false,
          preserveTailStartID: preserved.id,
        })
        ...
        const rt = runtime(
          ...
          Plugin.defaultLayer,
          wide(),
          cfg({ tail_turns: 1, preserve_recent_tokens: 10_000 }),
        )
        try {
          const msgs = await svc.messages({ sessionID: session.id })
          const parent = msgs.at(-1)?.info.id
          expect(parent).toBeTruthy()
          expect(msgs.indexOf(msgs.find((message) => message.info.id === preserved.id)!)).toBeLessThan(
            msgs.indexOf(msgs.find((message) => message.info.id === selected.id)!),
          )
          await rt.runPromise(
            SessionCompaction.Service.use((svc) =>
              svc.process({
          ...
          const part = await lastCompactionPart(session.id)
          // preserved 的 ID 字典序更大，但它在完整消息数组中更早，所以必须成为最终截断锚点。
          expect(part?.tail_start_id).toBe(preserved.id)
        } finally {
          await rt.dispose()
        }
      },
  ...
  test("falls back to the natural tail when a preserved anchor is missing", async () => {
    await using tmp = await tmpdir()
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const session = await svc.create({})
        await user(session.id, "history")
        const selected = await user(session.id, "natural recent tail")
        await SessionCompaction.create({
          sessionID: session.id,
          agent: "build",
          model: ref,
          auto: false,
          preserveTailStartID: MessageID.make("msg_missing_deferred"),
        })
        ...
        try {
          ...
          const part = await lastCompactionPart(session.id)
          expect(part?.tail_start_id).toBe(selected.id)
        } finally {
          await rt.dispose()
        }
      },
  ...
  test("replays the overflowing user turn after preflight auto compaction", async () => {
    await using tmp = await tmpdir()
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const session = await svc.create({})
        await user(session.id, "root")
        const replay = await user(session.id, "继续根据上面的要求生成 8 张图")
        await svc.updatePart({
          id: PartID.ascending(),
          messageID: replay.id,
          sessionID: session.id,
        ...
        try {
          ...
          expect(last?.parts.some((part) => part.type === "file")).toBe(false)
          expect(
            last?.parts.some((part) => part.type === "text" && part.text.includes("继续根据上面的要求生成 8 张图")),
          ).toBe(true)
          expect(
            last?.parts.some((part) => part.type === "text" && part.text.includes("Attached image/png: large.png")),
          ).toBe(true)
          expect(
            all.some((message) =>
              message.parts.some(
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        try {
          ...
          // 1000→3000ms：CI 上 retry status event 派发比 1s 慢
          await Promise.race([
            ready.promise,
            wait(5000).then(() => {
          ...
          abort.abort()
          // 250→2000ms：250ms 太紧 race；2s 仍显著快于 retry-after 10s backoff
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        try {
          ...
          // 1000→3000ms：CI 上 plugin/hook 派发比 1s 慢
          await Promise.race([
            ready.promise,
            wait(5000).then(() => {
...
describe("util.token.estimate", () => {
  ...
  test("keeps legacy non-ascii estimate for existing compaction behavior", () => {
    // 全局 estimate 仍保持旧的 4 字符约 1 token，避免已有中文会话刷新后因为算法变化突然触发压缩。
    expect(Token.estimate("上下文过长".repeat(1000))).toBe(1250)
  })

  test("estimates request bodies conservatively for empty-stream overflow detection", () => {
    // 只有空流上下文溢出兜底使用更保守的请求体估算，避免把必失败的长中文请求反复重试。
    expect(Token.estimateRequest("上下文过长".repeat(1000))).toBe(5000)
  })
})
