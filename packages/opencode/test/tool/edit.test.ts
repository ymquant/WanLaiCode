...
describe("tool.edit abort", () => {
  test("skips formatter and diagnostics once the run is aborted", async () => {
    ...
    const filepath = path.join(tmp.path, "aborted.cs")
    const marker = path.join(tmp.path, "formatter-ran")
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const result = await Effect.runPromise(
          edit.execute(
            { filePath: filepath, oldString: "using System;", newString: "using Up;" },
            { ...ctx, abort: AbortSignal.abort() },
          ),
        )

        // 写盘照常完成：中断只跳过收尾，不会留下半个文件
        expect(await fs.readFile(filepath, "utf-8")).toBe("using Up;\n")
