...
describe("tool.write", () => {
  ...
  describe("abort", () => {
    it.instance(
      "skips formatter and diagnostics once the run is aborted",
      () =>
        Effect.gen(function* () {
          const test = yield* TestInstance
          const filepath = path.join(test.directory, "aborted.cs")
          const marker = path.join(test.directory, "formatter-ran")

          const result = yield* run({ filePath: filepath, content: "using Up;\n" }, { ...ctx, abort: AbortSignal.abort() })

          // 写盘照常完成：中断只跳过收尾，不会留下半个文件
          expect(yield* Effect.promise(() => fs.readFile(filepath, "utf-8"))).toBe("using Up;\n")
          // 收尾（格式化 / LSP 诊断）是秒级等待，中断后必须直接跳过
          expect(yield* Effect.promise(() => fs.access(marker).then(() => true, () => false))).toBe(false)
          ...
          formatter: {
            marker: {
              ...
              command: [
                "node",
                "-e",
                "const path = require('path'); require('fs').writeFileSync(path.join(path.dirname(process.argv[1]), 'formatter-ran'), '1')",
                "$FILE",
              ],
            },
          },
