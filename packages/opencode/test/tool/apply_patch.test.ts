...
describe("tool.apply_patch freeform", () => {
  ...
  test("skips formatter and diagnostics once the run is aborted", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const target = path.join(fixture.path, "patched.cs")
        const marker = path.join(fixture.path, "formatter-ran")
        await fs.writeFile(target, "using System;\n", "utf-8")

        const patchText =
          "*** Begin Patch\n*** Update File: patched.cs\n@@\n-using System;\n+using Up;\n*** End Patch"
        const result = await execute({ patchText }, { ...ctx, abort: AbortSignal.abort() })

        // 写盘照常完成：中断只跳过收尾，不会留下半个文件
        expect(await fs.readFile(target, "utf-8")).toBe("using Up;\n")
