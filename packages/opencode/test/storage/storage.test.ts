...
describe("Storage.readOrQuarantine", () => {
  it.live("损坏文件被移到 .corrupt 并按 NotFound 处理", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const tmp = yield* tmpdirScoped()
      const file = path.join(tmp, "x.json")
      yield* fs.writeWithDirs(file, "{ broken")

      const exit = yield* Effect.exit(Storage.readOrQuarantine(fs, file))
      expect(Exit.isFailure(exit)).toBe(true)
      expect(yield* fs.isFile(file)).toBe(false)
      expect(yield* fs.isFile(file + ".corrupt")).toBe(true)
    }),
  )
  ...
  it.live("完整文件正常返回", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(file, JSON.stringify({ a: 1 }))

      const value = yield* Storage.readOrQuarantine(fs, file)
