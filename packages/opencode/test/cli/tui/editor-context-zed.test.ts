...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = (process.env.WANLAICODE_ZED_DB ?? process.env.OPENCODE_ZED_DB)
  process.env.WANLAICODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) {
    delete process.env.WANLAICODE_ZED_DB
    delete process.env.OPENCODE_ZED_DB
  }
    else process.env.WANLAICODE_ZED_DB = previous
    home.mockRestore()
  }
})
