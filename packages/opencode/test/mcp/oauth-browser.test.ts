...
test("BrowserOpenFailed event is NOT published when open() succeeds", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      // Poll for open() being called (1s interval, 5s timeout) instead of a fixed
      // 2s sleep. CI is sometimes too slow to traverse Config.get() + transport
      // start + redirectToAuthorization within 2s, which caused flakes.
      const deadline = Date.now() + 5_000
      while (openCalledWith === undefined && Date.now() < deadline) {
        await new Promise((resolve) => setTimeout(resolve, 1_000))
      }
      // Give the ~500ms open() error-detection window a chance to elapse before stop().
...
test("open() is called with the authorization URL", async () => {
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      // Poll quickly so callback shutdown can happen while authenticate is still
      // inside open()'s subprocess error-detection window.
      const deadline = Date.now() + 5_000
      while (openCalledWith === undefined && Date.now() < deadline) {
