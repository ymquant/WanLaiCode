...
// Import modules after mocking. The query suffix gives this file its own Bun
// module cache entry so other MCP tests cannot reuse a differently mocked MCP module.
const mcpModule = "../../src/mcp/index?test=mcp-oauth-auto-connect"
const { MCP }: typeof import("../../src/mcp/index") = await import(mcpModule)
const { McpOAuthCallback } = await import("../../src/mcp/oauth-callback")
const { provideTestInstance, tmpdir } = await import("../fixture/fixture")
  ...
  const port = await new Promise<number>((resolve, reject) => {
    ...
    server.listen(0, () => {
      ...
      if (!address || typeof address === "string") {
        server.close(() => reject(new Error("No available OAuth callback test port")))
        return
      }
...
const waitForSettledStatus = (
  status: () => Effect.Effect<Record<string, { status: string; error?: string }>>,
  name: string,
  attempt = 0,
): Effect.Effect<Record<string, { status: string; error?: string }>> =>
  Effect.gen(function* () {
    const current = yield* status()
    if (current[name]?.status !== "connecting" || attempt >= 20) return current
    yield* Effect.sleep("25 millis")
    return yield* waitForSettledStatus(status, name, attempt + 1)
  })
...
test("authenticate() stores a connected client when auth completes without redirect", async () => {
  const redirectUri = await testRedirectUri()
  ...
  await provideTestInstance({
    ...
    fn: async () => {
      await Effect.runPromise(
        MCP.Service.use((mcp) =>
          Effect.gen(function* () {
            const before = yield* waitForSettledStatus(mcp.status, "test-oauth-connect")
