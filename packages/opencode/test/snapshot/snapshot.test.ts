...
test("diffFull omits patch bodies before loading oversized text changes", async () => {
  await using tmp = await bootstrap()
  await WithInstance.provide({
    directory: tmp.path,
    fn: async () => {
      const file = fwd(tmp.path, "generated.txt")
      // 20,002 条增删行越过正文门禁；仍需返回精确文件状态和 numstat，供会话摘要展示。
      await Filesystem.write(file, Array.from({ length: 10_001 }, (_, index) => `before-${index}`).join("\n"))
      const before = await run(tmp.path, (snapshot) => snapshot.track())
      await Filesystem.write(file, Array.from({ length: 10_001 }, (_, index) => `after-${index}`).join("\n"))
      const after = await run(tmp.path, (snapshot) => snapshot.track())
...
test(
  ...
  async () => {
    ...
    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        // diffFull processes 100 rows per batch; 101 is the smallest fixture that exercises two batches.
