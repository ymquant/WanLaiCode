...
const originalWorkspacesFlag = Flag.WANLAICODE_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  OPENCODE_AUTH_CONTENT: process.env.WANLAICODE_AUTH_CONTENT ?? process.env.OPENCODE_AUTH_CONTENT,
...
afterEach(async () => {
  ...
  Flag.WANLAICODE_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
const waitForWorkspaceSync = (
  ...
  signal?: AbortSignal,
  timeoutMs?: number,
) => runWorkspace(WorkspaceOld.Service.use((workspace) => workspace.waitForSync(workspaceID, state, signal, timeoutMs)))
...
async function eventually<T>(fn: () => T | Promise<T>, timeout = 5000) {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.WANLAICODE_AUTH_CONTENT = JSON.stringify({
        ...
        // 工作区应只继承模型认证，远控设备令牌必须留在本机主进程。
        "wanlaicode-remote-control:account": { type: "wellknown", key: "desktop", token: "device-secret" },
      })
...
describe("workspace-old sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.WANLAICODE_EXPERIMENTAL_WORKSPACES = false
...
describe("workspace-old waitForSync", () => {
  ...
  test("times out with the requested fence in the error message", async () => {
    await withInstance(async () => {
      ...
      await expect(
        waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_timeout"), { [sessionID]: 1 }, undefined, 100),
