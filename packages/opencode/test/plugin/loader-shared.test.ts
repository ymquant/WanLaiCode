...
const disableDefault = (process.env.WANLAICODE_DISABLE_DEFAULT_PLUGINS ?? process.env.OPENCODE_DISABLE_DEFAULT_PLUGINS)
process.env.WANLAICODE_DISABLE_DEFAULT_PLUGINS = "1"
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.WANLAICODE_DISABLE_DEFAULT_PLUGINS; delete process.env.OPENCODE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.WANLAICODE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
async function load(dir: string) {
  const source = path.join(dir, "wanlaicode.json")
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "wanlaicode.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "wanlaicode.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "wanlaicode.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "wanlaicode.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "wanlaicode.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = (process.env.WANLAICODE_PURE ?? process.env.OPENCODE_PURE)
    process.env.WANLAICODE_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.WANLAICODE_PURE; delete process.env.OPENCODE_PURE
      } else {
        process.env.WANLAICODE_PURE = pure
      }
    }
  })
