...
function concurrentCount(defaultCount: number) {
  return process.platform === "win32" ? Math.min(defaultCount, 4) : defaultCount
}

const timeout = process.platform === "win32" ? 60_000 : 25_000
...
describe("plugin.install.concurrent", () => {
  test("serializes concurrent server config updates across processes", async () => {
    ...
    const all = mods("mod-server", concurrentCount(12))
    ...
    const cfg = await read(path.join(tmp.path, ".wanlaicode", "wanlaicode.jsonc"))
  ...
  test("serializes concurrent server+tui config updates across processes", async () => {
    ...
    const all = mods("mod-both", concurrentCount(10))
  ...
  test("preserves updates when existing config uses .json", async () => {
    ...
    const next = mods("mod-json", concurrentCount(8))
    const out = await Promise.all(
      next.map((mod) =>
        run({
          dir: tmp.path,
