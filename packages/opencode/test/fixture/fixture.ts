...
export async function disposeTestInstance(input: { directory: string; reason?: string }) {
  const ctx = await runTestInstanceStore((store) => store.load({ directory: input.directory }))
  return runTestInstanceStore((store) => store.dispose(ctx, input.reason))
}
...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.git) {
    ...
    if (options.git !== "no-commit") {
      await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
    }
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "wanlaicode.json"),
      JSON.stringify({
...
export function tmpdirScoped(options?: { git?: boolean | "no-commit"; config?: Partial<Config.Info> }) {
...
export function provideTmpdirInstance<A, E, R>(
  ...
  options?: { git?: boolean | "no-commit"; config?: Partial<Config.Info> },
) {
  return Effect.gen(function* () {
...
> {
  return Effect.gen(function* () {
    ...
    // Suggestions fire a background LLM request after every turn, which would
    // race hit/call-count assertions. Off by default; suggestion tests opt in.
    const config = options?.config ? { prompt_suggestions: false, ...options.config(llm.url) } : undefined
