import { test, expect, describe } from "bun:test"
import { Effect, Layer } from "effect"
import path from "path"
import fs from "fs/promises"
import { randomUUID } from "crypto"
import { Filesystem } from "@/util/filesystem"
import { File } from "../../src/file"
import { Skill } from "../../src/skill"
import { Instance } from "../../src/project/instance"
import { WithInstance } from "../../src/project/with-instance"
import { containsPath } from "../../src/project/instance-context"
import { provideInstance, tmpdir } from "../fixture/fixture"
import { Global } from "@opencode-ai/core/global"

const run = <A, E>(eff: Effect.Effect<A, E, File.Service>) =>
  Effect.runPromise(provideInstance(Instance.directory)(eff.pipe(Effect.provide(File.defaultLayer))))
const runWithSkill = <A, E>(location: string, eff: Effect.Effect<A, E, File.Service | Skill.Service>) =>
  Effect.runPromise(
    provideInstance(Instance.directory)(
      eff.pipe(
        Effect.provide(
          Layer.mergeAll(
            File.defaultLayer,
            Layer.succeed(
              Skill.Service,
              Skill.Service.of({
                get: () => Effect.succeed(undefined),
                all: () =>
                  Effect.succeed([
                    {
                      name: "skill-preview",
                      description: "Preview skill file.",
                      location,
                      content: "# Preview Skill",
                    },
                  ]),
                dirs: () => Effect.succeed([path.dirname(location)]),
                available: () => Effect.succeed([]),
                invalidate: () => Effect.void,
              }),
            ),
          ),
        ),
      ),
    ),
  )
const read = (file: string) => run(File.Service.use((svc) => svc.read(file)))
const readWithSkill = (location: string) => runWithSkill(location, File.Service.use((svc) => svc.read(location)))
const readWithSkillAlias = (location: string, file: string) =>
  runWithSkill(location, File.Service.use((svc) => svc.read(file)))
const list = (dir?: string) => run(File.Service.use((svc) => svc.list(dir)))

describe("Filesystem.contains", () => {
  test("allows paths within project", () => {
    expect(Filesystem.contains("/project", "/project/src")).toBe(true)
    expect(Filesystem.contains("/project", "/project/src/file.ts")).toBe(true)
    expect(Filesystem.contains("/project", "/project")).toBe(true)
  })

  test("blocks ../ traversal", () => {
    expect(Filesystem.contains("/project", "/project/../etc")).toBe(false)
    expect(Filesystem.contains("/project", "/project/src/../../etc")).toBe(false)
    expect(Filesystem.contains("/project", "/etc/passwd")).toBe(false)
  })

  test("blocks absolute paths outside project", () => {
    expect(Filesystem.contains("/project", "/etc/passwd")).toBe(false)
    expect(Filesystem.contains("/project", "/tmp/file")).toBe(false)
    expect(Filesystem.contains("/home/developer/project", "/home/developer/other")).toBe(false)
  })

  test("handles prefix collision edge cases", () => {
    expect(Filesystem.contains("/project", "/project-other/file")).toBe(false)
    expect(Filesystem.contains("/project", "/projectfile")).toBe(false)
  })
})

/*
 * Integration tests for read() and list() path traversal protection.
 *
 * These tests verify the HTTP API code path is protected. The HTTP endpoints
 * in server.ts (GET /file/content, GET /file) call read()/list()
 * directly - they do NOT go through ReadTool or the agent permission layer.
 *
 * This is a SEPARATE code path from ReadTool, which has its own checks.
 */
describe("File.read path traversal protection", () => {
  test("rejects ../ traversal attempting to read /etc/passwd", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "allowed.txt"), "allowed content")
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(read("../../../etc/passwd")).rejects.toThrow("Access denied: path escapes project directory")
      },
    })
  })

  test("rejects deeply nested traversal", async () => {
    await using tmp = await tmpdir()

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(read("src/nested/../../../../../../../etc/passwd")).rejects.toThrow(
          "Access denied: path escapes project directory",
        )
      },
    })
  })

  test("allows valid paths within project", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "valid.txt"), "valid content")
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const result = await read("valid.txt")
        expect(result.content).toBe("valid content")
      },
    })
  })

  test("rejects a project symlink to a file outside the project", async () => {
    if (process.platform === "win32") return
    const outsideName = `outside-${randomUUID()}.txt`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const outside = path.join(dir, "..", outsideName)
        await Bun.write(outside, "outside content")
        await fs.symlink(outside, path.join(dir, "linked.txt"))
      },
      dispose: async () => {
        await fs.rm(path.join(tmp.path, "..", outsideName), { force: true })
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(read("linked.txt")).rejects.toThrow("Access denied: path escapes project directory")
      },
    })
  })

  test("rejects a project symlink to a directory outside the project", async () => {
    const outsideName = `outside-${randomUUID()}`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const outside = path.join(dir, "..", outsideName)
        await fs.mkdir(outside, { recursive: true })
        await Bun.write(path.join(outside, "secret.txt"), "outside content")
        await fs.symlink(outside, path.join(dir, "linked-dir"), process.platform === "win32" ? "junction" : "dir")
      },
      dispose: async (dir) => {
        await fs.rm(path.join(dir, "..", outsideName), { force: true, recursive: true })
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(read(path.join("linked-dir", "secret.txt"))).rejects.toThrow(
          "Access denied: path escapes project directory",
        )
      },
    })
  })

  test("rejects listing a project symlink to a directory outside the project", async () => {
    const outsideName = `outside-list-${randomUUID()}`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const outside = path.join(dir, "..", outsideName)
        await fs.mkdir(outside, { recursive: true })
        await Bun.write(path.join(outside, "secret.txt"), "outside content")
        await fs.symlink(outside, path.join(dir, "linked-dir"), process.platform === "win32" ? "junction" : "dir")
      },
      dispose: async (dir) => {
        await fs.rm(path.join(dir, "..", outsideName), { force: true, recursive: true })
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(list("linked-dir")).rejects.toThrow("Access denied: path escapes project directory")
      },
    })
  })

  test("rejects exists on a project symlink to a directory outside the project", async () => {
    const outsideName = `outside-exists-${randomUUID()}`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const outside = path.join(dir, "..", outsideName)
        await fs.mkdir(outside, { recursive: true })
        await Bun.write(path.join(outside, "secret.txt"), "outside content")
        await fs.symlink(outside, path.join(dir, "linked-dir"), process.platform === "win32" ? "junction" : "dir")
      },
      dispose: async (dir) => {
        await fs.rm(path.join(dir, "..", outsideName), { force: true, recursive: true })
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const exists = await run(File.Service.use((svc) => svc.exists("linked-dir")))
        expect(exists).toEqual({ exists: false })
      },
    })
  })

  test("allows Codex system imagegen skill preview file outside project", async () => {
    await using tmp = await tmpdir()
    const skillPath = path.join(Global.Path.home, ".codex", "skills", ".system", "imagegen", "SKILL.md")
    await Bun.write(skillPath, "# Imagegen Preview")

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const result = await read(skillPath)
        expect(result.content).toBe("# Imagegen Preview")
      },
    })
  })

  test("allows discovered skill files outside project", async () => {
    const skillHome = `skill-home-${randomUUID()}`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const skillDir = path.join(dir, "..", skillHome)
        await fs.mkdir(skillDir, { recursive: true })
        await Bun.write(path.join(skillDir, "SKILL.md"), "# Preview Skill")
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const location = path.join(tmp.path, "..", skillHome, "SKILL.md")
        const result = await readWithSkill(location)
        expect(result.content).toBe("# Preview Skill")
      },
    })
  })

  test("allows discovered skill files through symlink-equivalent paths", async () => {
    const linkType = process.platform === "win32" ? "junction" : undefined
    const skillRoot = `skill-link-${randomUUID()}`
    await using tmp = await tmpdir({
      init: async (dir) => {
        const realDir = path.join(dir, "..", skillRoot, "skill-real")
        await fs.mkdir(realDir, { recursive: true })
        await Bun.write(path.join(realDir, "SKILL.md"), "# Symlink Skill")
        await fs.symlink(realDir, path.join(dir, "..", skillRoot, "skill-link"), linkType)
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const real = path.join(tmp.path, "..", skillRoot, "skill-real", "SKILL.md")
        const alias = path.join(tmp.path, "..", skillRoot, "skill-link", "SKILL.md")
        const result = await readWithSkillAlias(real, alias)
        expect(result.content).toBe("# Symlink Skill")
      },
    })
  })
})

describe("File.list path traversal protection", () => {
  test("rejects ../ traversal attempting to list /etc", async () => {
    await using tmp = await tmpdir()

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        await expect(list("../../../etc")).rejects.toThrow("Access denied: path escapes project directory")
      },
    })
  })

  test("allows valid subdirectory listing", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "subdir", "file.txt"), "content")
      },
    })

    await WithInstance.provide({
      directory: tmp.path,
      fn: async () => {
        const result = await list("subdir")
        expect(Array.isArray(result)).toBe(true)
      },
    })
  })
})

describe("containsPath", () => {
  test("returns true for path inside directory", async () => {
    await using tmp = await tmpdir({ git: true })

    await WithInstance.provide({
      directory: tmp.path,
      fn: () => {
        expect(containsPath(path.join(tmp.path, "foo.txt"), Instance.current)).toBe(true)
        expect(containsPath(path.join(tmp.path, "src", "file.ts"), Instance.current)).toBe(true)
      },
    })
  })

  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    await using tmp = await tmpdir({ git: true })
    const subdir = path.join(tmp.path, "packages", "lib")
    await fs.mkdir(subdir, { recursive: true })

    await WithInstance.provide({
      directory: subdir,
      fn: () => {
        // .wanlaicode at worktree root, but we're running from packages/lib
        expect(containsPath(path.join(tmp.path, ".wanlaicode", "state"), Instance.current)).toBe(true)
        // sibling package should also be accessible
        expect(containsPath(path.join(tmp.path, "packages", "other", "file.ts"), Instance.current)).toBe(true)
        // worktree root itself
        expect(containsPath(tmp.path, Instance.current)).toBe(true)
      },
    })
  })

  test("returns false for path outside both directory and worktree", async () => {
    await using tmp = await tmpdir({ git: true })
    const outsideFile = path.join(tmp.path, "..", "other-project", "file.txt")

    await WithInstance.provide({
      directory: tmp.path,
      fn: () => {
        expect(containsPath(outsideFile, Instance.current)).toBe(false)
        expect(containsPath(path.dirname(outsideFile), Instance.current)).toBe(false)
      },
    })
  })

  test("returns false for path with .. escaping worktree", async () => {
    await using tmp = await tmpdir({ git: true })

    await WithInstance.provide({
      directory: tmp.path,
      fn: () => {
        expect(containsPath(path.join(tmp.path, "..", "escape.txt"), Instance.current)).toBe(false)
      },
    })
  })

  test("handles directory === worktree (running from repo root)", async () => {
    await using tmp = await tmpdir({ git: true })
    const outsideFile = path.join(tmp.path, "..", "outside-root", "file.txt")

    await WithInstance.provide({
      directory: tmp.path,
      fn: () => {
        expect(Instance.directory).toBe(Instance.worktree)
        expect(containsPath(path.join(tmp.path, "file.txt"), Instance.current)).toBe(true)
        expect(containsPath(outsideFile, Instance.current)).toBe(false)
      },
    })
  })

  test("non-git project does not allow arbitrary paths via worktree='/'", async () => {
    await using tmp = await tmpdir() // no git: true
    const outsideFile = path.join(tmp.path, "..", "other", "file.txt")

    await WithInstance.provide({
      directory: tmp.path,
      fn: () => {
        // worktree is "/" for non-git projects, but containsPath should NOT allow all paths
        expect(containsPath(path.join(tmp.path, "file.txt"), Instance.current)).toBe(true)
        expect(containsPath(outsideFile, Instance.current)).toBe(false)
        expect(containsPath(path.dirname(outsideFile), Instance.current)).toBe(false)
      },
    })
  })
})
