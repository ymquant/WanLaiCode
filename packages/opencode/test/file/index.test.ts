...
describe("file/index Filesystem patterns", () => {
  ...
  describe("read() - binary content", () => {
    ...
    test.each(["large.mp4", "large.mp3"])("does not inline oversized media %s", async (name) => {
      await using tmp = await tmpdir()
      await fs.writeFile(path.join(tmp.path, name), Buffer.alloc(20 * 1024 * 1024 + 1))
  ...
  describe("shouldEncode() logic", () => {
    ...
    test("does not inline oversized preview files", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "large.docx")
      ...
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          const result = await read("large.docx")
