...
describe("Worktree", () => {
      ...
      provideTmpdirInstance(
        () =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe(`wanlaicode/${info.name}`)
      ...
      provideTmpdirInstance(
        () =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("wanlaicode/my-feature")
          }),
        { git: true },
      ),
    )
  ...
  describe("create + remove lifecycle", () => {
      ...
      provideTmpdirInstance(
        () =>
          Effect.gen(function* () {
            ...
            expect(info.name).toBeDefined()
            expect(info.branch).toStartWith("wanlaicode/")
    ...
    it.live("create succeeds when source repo has no commits (unborn HEAD)", () =>
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            const svc = yield* Worktree.Service
            ...
            const next = yield* Effect.promise(() => fs.realpath(info.directory).catch(() => info.directory))
            expect(normalize(list)).toContain(normalize(next))
      ...
      provideTmpdirInstance(
        () =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("wanlaicode/test-workspace")
