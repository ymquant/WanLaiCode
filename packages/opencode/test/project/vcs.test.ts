...
describe("Vcs diff", () => {
  ...
  test("diff('unstaged') returns uncommitted changes", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    await withVcsOnly(tmp.path, async () => {
      const diff = await AppRuntime.runPromise(
        Effect.gen(function* () {
          const vcs = yield* Vcs.Service
          return yield* vcs.diff("unstaged")
        }),
      )
      expect(diff).toEqual(
  ...
  test("diff('unstaged') keeps batched patches aligned for type changes", async () => {
  ...
  test("diff('unstaged') keeps carriage returns inside patch hunks", async () => {
    if (process.platform === "win32") return
...
describe("Vcs commit", () => {
  ...
  test("stages only listed files when stageAll is false", async () => {
    await using tmp = await tmpdir({ git: true })
    await fs.writeFile(path.join(tmp.path, "a.txt"), "original\n", "utf-8")
    await fs.writeFile(path.join(tmp.path, "b.txt"), "original\n", "utf-8")
    await $`git add .`.cwd(tmp.path).quiet()
    await $`git commit --no-gpg-sign -m "initial"`.cwd(tmp.path).quiet()
    await fs.writeFile(path.join(tmp.path, "a.txt"), "changed a\n", "utf-8")
    await fs.writeFile(path.join(tmp.path, "b.txt"), "changed b\n", "utf-8")

    await withVcsOnly(tmp.path, async () => {
      const result = await AppRuntime.runPromise(
        Effect.gen(function* () {
          const vcs = yield* Vcs.Service
          return yield* vcs.commit({ message: "partial", stageAll: false, files: ["a.txt"] })
        }),
      )
      expect(result.hash).toBeTruthy()

      const headA = await $`git show HEAD:a.txt`.cwd(tmp.path).quiet().text()
      expect(headA).toBe("changed a\n")
  ...
  test("stages all changes when stageAll is true", async () => {
    await using tmp = await tmpdir({ git: true })
    await fs.writeFile(path.join(tmp.path, "a.txt"), "original\n", "utf-8")
    await $`git add .`.cwd(tmp.path).quiet()
    await $`git commit --no-gpg-sign -m "initial"`.cwd(tmp.path).quiet()
    await fs.writeFile(path.join(tmp.path, "a.txt"), "changed\n", "utf-8")
    await fs.writeFile(path.join(tmp.path, "b.txt"), "new\n", "utf-8")
    ...
    await withVcsOnly(tmp.path, async () => {
      await AppRuntime.runPromise(
        Effect.gen(function* () {
          const vcs = yield* Vcs.Service
          return yield* vcs.commit({ message: "all", stageAll: true })
        }),
      )
  ...
  test("commits only staged changes when stageAll is false and files are omitted", async () => {
    await using tmp = await tmpdir({ git: true })
    await fs.writeFile(path.join(tmp.path, "a.txt"), "original\n", "utf-8")
    await fs.writeFile(path.join(tmp.path, "b.txt"), "original\n", "utf-8")
    await $`git add .`.cwd(tmp.path).quiet()
    await $`git commit --no-gpg-sign -m "initial"`.cwd(tmp.path).quiet()
    await fs.writeFile(path.join(tmp.path, "a.txt"), "staged\n", "utf-8")
    await fs.writeFile(path.join(tmp.path, "b.txt"), "unstaged\n", "utf-8")
    await $`git add a.txt`.cwd(tmp.path).quiet()

    await withVcsOnly(tmp.path, async () => {
      await AppRuntime.runPromise(
        Effect.gen(function* () {
          const vcs = yield* Vcs.Service
          return yield* vcs.commit({ message: "staged only", stageAll: false })
        }),
      )

      const headA = await $`git show HEAD:a.txt`.cwd(tmp.path).quiet().text()
      expect(headA).toBe("staged\n")
  ...
  test("pullRequestReadiness does not treat commits already on origin as unpushed", async () => {
    await using tmp = await tmpdir({ git: true })
    const bare = path.join(path.dirname(tmp.path), `origin-${Math.random().toString(36).slice(2)}.git`)
    await $`git init --bare ${bare}`.quiet()
    await $`git remote add origin ${bare}`.cwd(tmp.path).quiet()
    await $`git push -u origin HEAD`.cwd(tmp.path).quiet()
    await $`git checkout -b codex/1`.cwd(tmp.path).quiet()
    await fs.writeFile(path.join(tmp.path, "feature.txt"), "feature\n", "utf-8")
    await $`git add feature.txt`.cwd(tmp.path).quiet()
    await $`git commit --no-gpg-sign -m "feature"`.cwd(tmp.path).quiet()
    await $`git push origin HEAD:refs/heads/codex/111`.cwd(tmp.path).quiet()

    await withVcsOnly(tmp.path, async () => {
      const readiness = await AppRuntime.runPromise(
        Effect.gen(function* () {
          const vcs = yield* Vcs.Service
          return yield* vcs.pullRequestReadiness()
        }),
      )
      expect(readiness.branch).toBe("codex/1")
      expect(readiness.branch_on_remote).toBe(false)
      expect(readiness.unpushed_commits).toBe(false)
    })
