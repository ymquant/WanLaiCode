...
describe("Project.fromDirectory", () => {
  ...
  test("subdirectory without local .git inherits repo worktree and id but not vcs", async () => {
    await using tmp = await tmpdir({ git: true })
    const sub = path.join(tmp.path, "sub")
    await fs.mkdir(sub, { recursive: true })
    const { project: root } = await run((svc) => svc.fromDirectory(tmp.path))
    const { project, sandbox } = await run((svc) => svc.fromDirectory(sub))
    expect(project.id).toBe(root.id)
