...
import { expandWindowsEnv, mergePathSegments, which } from "../../src/util/which"
...
describe("util.which", () => {
  ...
  test("mergePathSegments deduplicates case-insensitively on windows", () => {
    if (process.platform !== "win32") return

    expect(mergePathSegments("C:\\Tools", "D:\\GitHubCLI", "c:\\tools")).toBe("C:\\Tools;D:\\GitHubCLI")
  })

  test("expandWindowsEnv expands registry-style placeholders", () => {
    expect(expandWindowsEnv("%USERPROFILE%\\bin", { USERPROFILE: "C:\\Users\\me" })).toBe("C:\\Users\\me\\bin")
  })

  test("finds gh from registry PATH on windows", () => {
    if (process.platform !== "win32") return
    ...
    expect(gh.toLowerCase()).toContain("gh.exe")
  })
})
