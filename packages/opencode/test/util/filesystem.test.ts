...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "wanlaicode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "wanlaicode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "wanlaicode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "wanlaicode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["wanlaicode.json", "wanlaicode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "wanlaicode.json"),
        path.join(project, "wanlaicode.jsonc"),
      ])
    })
  })
  ...
  describe("windowsPath()", () => {
    test("converts Git Bash paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/c/Users/test")).toBe("C:/Users/developer")
    ...
    test("converts Cygwin paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/cygdrive/c/Users/test")).toBe("C:/Users/developer")
    ...
    test("converts WSL paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/mnt/c/Users/test")).toBe("C:/Users/developer")
    ...
    test("ignores normal Windows paths", () => {
      expect(Filesystem.windowsPath("C:/Users/developer")).toBe("C:/Users/developer")
      expect(Filesystem.windowsPath("D:\\dev\\project")).toBe("D:\\dev\\project")
    })
  })
