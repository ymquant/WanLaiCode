...
// 精确请求门闩保存自己的匹配条件，避免累计调用数把无关后台请求计入目标回合。
type MatchWait = {
  match: Match
  ready: Deferred.Deferred<void>
}
...
function textLine(value: string, phase?: "commentary" | "final_answer") {
  // 测试服务器用扩展字段把 Responses 的 item phase 穿过通用 Reply 流，Chat Completions 用例会自然忽略它。
  return chunk({ delta: { content: value, ...(phase ? { phase } : {}) } })
}
...
function responseMessage(id: string, seq: number, phase?: "commentary" | "final_answer") {
  return {
    type: "response.output_item.added",
    sequence_number: seq,
    output_index: 0,
    item: { type: "message", id, ...(phase ? { phase } : {}) },
  }
}
...
function responseMessageDone(id: string, seq: number, phase?: "commentary" | "final_answer") {
  return {
...
function flow(item: Sse) {
  ...
  for (const part of [...item.head, ...item.tail]) {
    ...
    if (delta && "content" in delta && typeof delta.content === "string") {
      // 仅接受官方两个 phase 值，避免测试辅助层生成线上 schema 不可能出现的状态。
      const phase =
        "phase" in delta && (delta.phase === "commentary" || delta.phase === "final_answer") ? delta.phase : undefined
      out.push({ type: "text", text: delta.content, phase })
    }
...
function responses(item: Sse, model: string) {
  ...
  let msgPhase: "commentary" | "final_answer" | undefined
  ...
  for (const part of flow(item)) {
    if (part.type === "text") {
      ...
      if (!hasMsg) {
        hasMsg = true
        seq += 1
        lines.push(responseMessage(msg, seq, msgPhase))
      }
      seq += 1
  ...
  if (msg) {
    seq += 1
    lines.push(responseMessageDone(msg, seq, msgPhase))
  }
  if (reason && !item.omitReasonEnd) {
    seq += 1
...
export class Reply {
  ...
  commentary(value: string) {
    // 显式构造 Responses commentary，用于覆盖“有进度文字但没有最终回答”的真实流式边界。
    this.#tail = [...this.#tail, textLine(value, "commentary")]
    return this
  }
  ...
  reasonWithoutEnd(value: string) {
    // 构造 response.completed 已到达但 reasoning item 未单独 done 的网关边界，用于验证客户端补终态。
    this.#tail = [...this.#tail, reasonLine(value)]
    this.#omitReasonEnd = true
    return this
  }
  ...
  item(): Item {
    return {
      ...
      error: this.#error,
      reset: this.#reset,
      omitReasonEnd: this.#omitReasonEnd,
    }
  }
}
...
export function raw(input: {
  ...
  return {
    ...
    error: input.error,
    reset: input.reset,
    omitReasonEnd: input.omitReasonEnd,
  }
}
...
function isSuggestionRequest(body: unknown): boolean {
  if (!body || typeof body !== "object") return false
  return JSON.stringify(body).includes("Predict the user's next prompt")
}
...
namespace TestLLMServer {
  export interface Service {
    ...
    /** 等待第一条满足正文/协议判据的真实 HTTP 请求。 */
    readonly waitMatch: (match: Match) => Effect.Effect<void>
...
export class TestLLMServer extends Context.Service<TestLLMServer, TestLLMServer.Service>()("@test/LLMServer") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      let list: Queue[] = []
      let waits: Wait[] = []
      // 内容门闩与累计请求计数分离，后台标题、建议和重试请求不能提前释放指定回合的测试。
      let matchWaits: MatchWait[] = []
      let misses: Hit[] = []
      ...
      const notify = Effect.fnUntraced(function* () {
        ...
        // 每个匹配门闩只在真实命中其请求 body 后释放，避免 llm.wait(n) 被无关后台请求满足。
        const matched = matchWaits.filter((item) => hits.some(item.match))
        matchWaits = matchWaits.filter((item) => !hits.some(item.match))
        yield* Effect.forEach(matched, (item) => Deferred.succeed(item.ready, void 0))
      })
      ...
      // Only consumes entries with an explicit match — background requests
      // (e.g. suggestion generation) must never steal unmatched queued replies.
      const pullMatched = (hit: Hit) => {
        const index = list.findIndex((entry) => entry.match?.(hit))
        if (index === -1) return
        const first = list[index]
        list = [...list.slice(0, index), ...list.slice(index + 1)]
        return first.item
      ...
      const handle = Effect.fn("TestLLMServer.handle")(function* (mode: "chat" | "responses") {
        ...
        if (isSuggestionRequest(body)) {
          ...
          // Auto-respond with the no-suggestion sentinel so nothing is published.
          const auto: Sse = { type: "sse", head: [role()], tail: [textLine("NONE"), finishLine("stop")] }
          if (mode === "responses") return send(responses(auto, modelFrom(body)))
          return send(auto)
        }
      ...
      return TestLLMServer.of({
        ...
        waitMatch: Effect.fn("TestLLMServer.waitMatch")(function* (match: Match) {
          // 先同步创建并登记门闩，再统一检查已有 hit；请求落在调用边界时也不会错过通知。
          const ready = Deferred.makeUnsafe<void>()
          matchWaits = [...matchWaits, { match, ready }]
          yield* notify()
          yield* Deferred.await(ready)
        }),
